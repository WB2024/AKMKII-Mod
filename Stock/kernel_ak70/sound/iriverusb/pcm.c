...
extern int set_xmos_is_playing(int playing);
extern void set_xmos_cur_sample_rate(int rate);
extern void set_xmos_pwr_ctrl(int pwr);
...
static int set_format(struct snd_usb_substream *subs, struct audioformat *fmt)
{
	...
	if (subs->interface != fmt->iface ||
		...
		if (err < 0) {
			printk("%d:%d:%d: usb_set_interface failed (%d)\n",
				   dev->devnum, fmt->iface, fmt->altsetting, err);

#if (CUR_AK == MODEL_AK380 || CUR_AK == MODEL_AK300)
			if(dev->devnum >= 2 && err == -ETIMEDOUT)
			{
				set_xmos_pwr_ctrl(0);
				msleep_interruptible(12);
				printk("++++++++++++++++++++++++++++++++++++++++++++++++++++++++>>err:ETIMEDOUT\n");
				set_xmos_pwr_ctrl(1);
				// 2015.11.11 tonny : 충분한 딜레이 필요함  줄이지 말 것
			     ...
			     struct snd_pcm_hw_params *hw_params)
{
	...
	// 2015.05.25 tonny
	set_xmos_cur_sample_rate(subs->cur_rate);

#if 0//(CUR_AK == MODEL_AK380 || CUR_AK == MODEL_AK320 || CUR_AK == MODEL_AK300)        
    set_cpu_mem_down();         /* [downer] A150526 for DSD noise */
#endif
...
//J151113
printk("--%s:%d:rate(%d), format(0x%x)--\n", __func__, __LINE__, subs->cur_rate, subs->cur_audiofmt);
...
extern int get_ak_volume_mute(void); /* [downer] A150806 */
...
static int snd_usb_pcm_open(struct snd_pcm_substream *substream, int direction)
{
	...
	snd_usb_pcm_open_pre();
#endif

#if (CUR_AK == MODEL_AK380 || CUR_AK == MODEL_AK320 || CUR_AK == MODEL_AK300)
    /* [downer] A150806 Volume 0 ?곹깭?먯꽌 DSD ?ъ깮 ??Mute ?쒓굅.*/
    if (get_ak_volume_mute()) {
        //ak_set_hw_mute(AMT_HP_MUTE, 0);
        printk("mute rejection\n");
    }
#endif
...
#ifdef PCM_DSD_CALLBACK
	snd_usb_pcm_open_post();
#endif
	
	//2015.05.11 tonny
	set_xmos_is_playing(1);
...
static int snd_usb_pcm_close(struct snd_pcm_substream *substream, int direction)
{
	...
	//2015.05.11 tonny
	set_xmos_is_playing(0);

    /* [downer] A140225 pcm close ??LCD off ?곹깭?대㈃ wakelock 3遺?..*/
					      ...
					      int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		...
		subs->running = 1;
		#ifdef PCM_DSD_CALLBACK
		snd_usb_playback_start_post();		
		#endif

		return 0;
	case SNDRV_PCM_TRIGGER_STOP:
		#ifdef PCM_DSD_CALLBACK		
		snd_usb_playback_stop_pre();
		#endif
