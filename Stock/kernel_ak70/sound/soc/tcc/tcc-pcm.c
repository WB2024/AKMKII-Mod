...
//    .formats = (SNDRV_PCM_FMTBIT_S16_LE | SNDRV_PCM_FMTBIT_U16_LE | SNDRV_PCM_FMTBIT_S24_LE | SNDRV_PCM_FMTBIT_IEC958_SUBFRAME_LE),	// Planet 20120306
	.formats = (SNDRV_PCM_FMTBIT_S16_LE | SNDRV_PCM_FMTBIT_U16_LE | SNDRV_PCM_FMTBIT_S24_LE | SNDRV_PCM_FMTBIT_S24_3LE), // Planet 20120306
...
static int tcc_pcm_hw_params(struct snd_pcm_substream *substream, struct snd_pcm_hw_params *params)
{
	...
	if (substream->pcm->device == __I2S_DEV_NUM__) {
		...
		#endif
		 if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		 if((params_format(params) == SNDRV_PCM_FORMAT_S16_LE) || (params_format(params) == SNDRV_PCM_FORMAT_U16_LE)) {

				 BITSET(pADMA->ChCtrl, Hw12);		 
				 BITCSET(pADMA_DAI->DAMR, (Hw20-Hw18), (Hw19|Hw18));

				 alsa_dbg("\n---------------------- S16 -------------------\n"); 
			 } 
			 else if((params_format(params) == SNDRV_PCM_FORMAT_S24_LE) || (params_format(params) == SNDRV_PCM_FORMAT_U24_LE) || (params_format(params) == SNDRV_PCM_FORMAT_S24_3LE)) {
				#ifdef PRINT_ALSA_MSG	
				 DBG_PRINT("TCCPCM","\n");
				#endif
				 BITCLR(pADMA->ChCtrl, Hw12);		 
				 BITCSET(pADMA_DAI->DAMR, (Hw20-Hw18), (Hw19)); 				 
				 alsa_dbg("\n---------------------- S24 -------------------\n");
			 } else {
				 /* To do*/
				 printk("Unspported Format Playback!\n");							 
			 }
		 } else {	 // Capture
			 if((params_format(params) == SNDRV_PCM_FORMAT_S16_LE) || (params_format(params) == SNDRV_PCM_FORMAT_U16_LE)) {
				 BITSET(pADMA->ChCtrl, Hw14);		 
				 BITCSET(pADMA_DAI->DAMR, (Hw22-Hw20), (Hw21|Hw20)); 
			 } else if((params_format(params) == SNDRV_PCM_FORMAT_S24_LE) || (params_format(params) == SNDRV_PCM_FORMAT_U24_LE) || (params_format(params) == SNDRV_PCM_FORMAT_S24_3LE)) {
				 BITCLR(pADMA->ChCtrl, Hw14);		 
				 BITCSET(pADMA_DAI->DAMR, (Hw22-Hw20), (Hw21)); 				 
			 } else {
				 /* To do*/
				 printk("Unspported Format Capture!\n");		 
			 }
...
#define USE_FADE_INOUT


#ifdef USE_FADE_INOUT
...
static int tcc_pcm_close(struct snd_pcm_substream *substream)
{
    ...
    /* [downer] A140225 pcm close 시 LCD off 상태이면 wakelock 3분...*/
