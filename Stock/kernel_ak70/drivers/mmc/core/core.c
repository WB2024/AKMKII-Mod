...
void mmc_request_done(struct mmc_host *host, struct mmc_request *mrq)
{
    ...
    if ((mrq == NULL) || (host == NULL)) {
        printk("%s: NULL return.\n", __func__);
        return;
    }
...
int mmc_flush_cache(struct mmc_card *card)
{
	...
	if (mmc_card_mmc(card) &&
		...
		if (err == -ETIMEDOUT) {
			printk("%s: cache flush timeout\n",
					mmc_hostname(card->host));
			rc = mmc_interrupt_hpi(card);
			if (rc) {
				printk("%s: mmc_interrupt_hpi() failed (%d)\n",
						mmc_hostname(host), rc);
				err = -ENODEV;
			}
		} else if (err) {
			printk("%s: cache flush error %d\n",
					mmc_hostname(card->host), err);
		}
		else
			printk("%s: cache flush success\n", mmc_hostname(card->host));
	}
	...
	if (card && mmc_card_mmc(card) &&
		...
		if (card->ext_csd.cache_ctrl ^ enable) {
			...
			if (err == -ETIMEDOUT && !enable) {
				printk("%s:cache disable operation timeout\n",
						mmc_hostname(card->host));
				rc = mmc_interrupt_hpi(card);
				if (rc)
