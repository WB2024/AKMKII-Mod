...
static int mmc_read_ext_csd(struct mmc_card *card, u8 *ext_csd)
{
	...
	case EXT_CSD_CARD_TYPE_DDR_52 | EXT_CSD_CARD_TYPE_52 |
		...
		card->ext_csd.hs_max_dtr = 48000000; //iriver-ysyim:2013.08.22
	...
	if (card->ext_csd.rev >= 6) {
		card->ext_csd.generic_cmd6_time = 10 * ext_csd[EXT_CSD_GENERIC_CMD6_TIME];
		printk("=========> Generic CMD6 time: %d\n", card->ext_csd.generic_cmd6_time);
	...
	struct mmc_card *oldcard)
{
	...
	if (card->ext_csd.rev >= 6) {
		if (card->ext_csd.cache_size > 0) {
			err = mmc_switch(card, EXT_CSD_CMD_SET_NORMAL,
							 EXT_CSD_CACHE_CTRL, 1, card->ext_csd.generic_cmd6_time);
			if (err && err != -EBADMSG) {
