...
#define SD_BOUNCE_DELAY       (3000*HZ/1000) /* 3s */
#define SD_POWEROFF_DELAY     (10 * HZ) /* 10s */

/*
* [downer] M130821
*/
struct sd30_detect_workqueue {
...
struct sd30_detect_workqueue *sd30_detect_wq;
static int g_set_sd30_detect_irq = 0;

#define USE_WAKELOCK

#ifdef USE_WAKELOCK
static struct wake_lock sd_wakelock;
#endif

static int g_emmc_hs_mode = 0;  /* [downer] A150930 */
static int g_wifi_state = 0;

static int g_after_wakeup = 0;  /* [downer] A140122 */
...
EXPORT_SYMBOL(get_wifi_state);

int is_emmc_hs_mode(void)
{
    return g_emmc_hs_mode;
}
EXPORT_SYMBOL(is_emmc_hs_mode);
...
extern int get_current_pcfi_state(void); /* [downer] A140510 */
/*
...
//			msleep(1);
			mdelay(1);	/* modified to fix WIFI kernel error on sleep/wakeup */
...
/* [downer] A140115 SDXC */
static int g_uhs_sdcard_insert = 0;
int get_uhs_sdcard_inserted(void)
{
    return g_uhs_sdcard_insert;
}
EXPORT_SYMBOL(get_uhs_sdcard_inserted);

static int g_sd_unstable_eject = 0;
void set_sd_unstable_eject(void)
{
    printk("\x1b[1;32m %s \x1b[0m\n", __func__);    
    g_sd_unstable_eject = 1;
}
EXPORT_SYMBOL(set_sd_unstable_eject);

static int get_sd_unstable_eject(void)
{
    return g_sd_unstable_eject;
}

/* [downer] A131230 real detect */
static void sd_bounce_process(unsigned long data)
{
	struct tcc_mmc_host *host = (struct tcc_mmc_host *)data;
    unsigned int status;
    ...
    sd30_detect_wq->state = status;
    
    if (status) {
        if (host->pdata->switch_voltage)
            host->pdata->switch_voltage(host->dev, host->id, SD30_ENABLE); /* SD I/O voltage enable */

        host->init_complete = 1;
        mmc_detect_change(host->mmc, msecs_to_jiffies(100));                        
    }
...
static void sd_poweroff_process(unsigned long data)
{
	struct tcc_mmc_host *host = (struct tcc_mmc_host *)data;

    if (host->pdata->switch_voltage)
        host->pdata->switch_voltage(host->dev, host->id, SD30_DISABLE); /* SD I/O voltage disable */

    /* [downer] M141021 */
    if (host->id == TCC_MMC_TYPE_SD) 
        clk_set_rate(host->fclk, host->mmc->f_min);            
}

/* [downer] A140510 SD eject in USB_DAC state */
static void sd_eject_process(unsigned long data)
{
	struct tcc_mmc_host *host = (struct tcc_mmc_host *)data;
    ...
    if (!get_current_pcfi_state()) {
        ...
        host->init_complete = 0;
        g_uhs_sdcard_insert = 0;
        g_sd_unstable_eject = 0;
        
        init_timer(&host->sd_poweroff_timer);
        host->sd_poweroff_timer.function = sd_poweroff_process;
        host->sd_poweroff_timer.data = (unsigned long)host;
        host->sd_poweroff_timer.expires = jiffies + SD_POWEROFF_DELAY;
    
        add_timer(&host->sd_poweroff_timer);
    }
    else
        mod_timer(&host->sd_eject_timer, jiffies + DETECT_TIMEOUT);        
}
...
static void tcc_mmc_check_status(struct tcc_mmc_host *host)
{
...
#ifdef USE_WAKELOCK
            wake_lock_timeout(&sd_wakelock, 10 * HZ); /* [downer] A140114 */
            printk(KERN_INFO "sd-detect wake lock ++\n");
#endif
        }
        
		if (status) {
            if (get_boot_complete()) {
                del_timer(&host->sd_poweroff_timer);
                
                /* [downer] A131230 bounce detect */
                init_timer(&host->bounce_timer);
                host->bounce_timer.function = sd_bounce_process;
                host->bounce_timer.data = (unsigned long)host;
                host->bounce_timer.expires = jiffies + SD_BOUNCE_DELAY;

                add_timer(&host->bounce_timer);
            }
            else {
                printk("\x1b[1;31m before boot complete \x1b[0m\n");
                if (host->pdata->switch_voltage)
                    host->pdata->switch_voltage(host->dev, host->id, SD30_ENABLE); /* SD I/O voltage enable */
            ...
            if (host->init_complete) {
                ...
                else {          /* [downer] A140510 for SD eject control in USB_DAC state */
                    init_timer(&host->sd_eject_timer);
                    host->sd_eject_timer.function = sd_eject_process;
                    host->sd_eject_timer.data = (unsigned long)host;
                    host->sd_eject_timer.expires = jiffies + DETECT_TIMEOUT;

                    add_timer(&host->sd_eject_timer);
                }
            }
		}
	}
	...
	schedule_work(&sd30_detect_wq->wq);
}
/*
...
/*
* [downer] A130821 SD detect IRQ level change 
*/
static void sd30_irq_change_handler(struct work_struct *work)
{
	struct sd30_detect_workqueue *p = container_of(work, struct sd30_detect_workqueue, wq);
    struct tcc_mmc_host *host = p->host;
	bool state = p->state;
	int irq = p->irq;

	if (g_set_sd30_detect_irq)
		free_irq(irq, host);

	g_set_sd30_detect_irq = 0;

	if (state) { 
		if (request_irq(irq, tcc_mmc_cd_irq, IRQ_TYPE_EDGE_RISING, "sd30_detect", host))
			printk("request rising edge of irq#%d failed!\n", irq);
		else 
			g_set_sd30_detect_irq = 1;
	}
	else { 
		if (request_irq(irq, tcc_mmc_cd_irq, IRQ_TYPE_EDGE_FALLING, "sd30_detect", host))
...
{	// ksjung	- vmmv
	...
	//long dwMaxClockRate = host->peri_clk;
	long dwMaxClockRate = 0;
	int i=0;
	u32 clk_div = 0;
	PSDCHCTRL sdctrl = (PSDCHCTRL)tcc_p2v(HwSDMMC_CHCTRL_BASE); 	

	if (ios->signal_voltage != MMC_SIGNAL_VOLTAGE_180) {
        /* [downer] A130911 for under SD v3.0 */
        if ((host->id == TCC_MMC_TYPE_SD) && (host->card_inserted)) {
			clk_set_rate(host->fclk, 48000000); /* clk freq fix 48MHz under SD v2.0 */
            if (host->pdata->switch_voltage)
                host->pdata->switch_voltage(host->dev, host->id, SD30_OFF); /* SD I/O voltage change to 3.3v */
        }
		return 0;
	}
	...
	if (ios->clock != 0) {
		...
		if (ios->signal_voltage == MMC_SIGNAL_VOLTAGE_180) {
			...
			if ((host->pdata->switch_voltage) && (host->card_inserted)) {
				host->pdata->switch_voltage(host->dev, host->id, SD30_ON);
                g_uhs_sdcard_insert = 1;
            }
...
static void init_mmc_host(struct tcc_mmc_host *host, unsigned int clock_rate)
{
	...
	host->peri_clk = clk_get_rate(host->fclk);
	SD_DEBUG("\x1b[1;31m tcc-sdhc%d: clock %lu \x1b[0m\n", host->id, host->peri_clk);
	...
	case 0:
		...
		sdctrl->SD1CAPREG0.nREG = 0x8CFF9870;
	...
	case 1: // SD
		...
		sdctrl->SD1CAPREG0.nREG = 0x2CFF9870;
		sdctrl->SD1CAPREG1.nREG = 0x00000007;
		
		break;
	case 2:                                  /* WiFi */
		sdctrl->SD2CMDDAT.nREG = 0x05050C0C;//0x3F3F3F3F;
		sdctrl->SD2CAPREG0.nREG = 0x0CFF9870;
		sdctrl->SD2CAPREG1.nREG = 0x00000007;            		
            
		break;
	case 3:
		sdctrl->SD3CMDDAT.nREG = 0x05050C0C;//0x0F0F0F0F;//3F3F3F3F;
		sdctrl->SD3CAPREG0.nREG = 0x8CFF9870;
...
static void tcc_mmc_early_suspend(struct early_suspend *h)
{

}

static void tcc_mmc_late_resume(struct early_suspend *h)
{
	struct tcc_mmc_host *host = container_of(h, struct tcc_mmc_host, early_suspend);

    if (host->id == TCC_MMC_TYPE_SD && g_after_wakeup) {
        g_after_wakeup = 0;
        tcc_mmc_check_status(host);
    }
}
...
//	host->cd_irq = platform_get_irq(pdev,  1);
	...
	if(host->id == TCC_MMC_TYPE_WIFI) 
	    mmc->pm_flags |= MMC_PM_KEEP_POWER;// | MMC_PM_WAKE_SDIO_IRQ;
    ...
    if ((host->id == TCC_MMC_TYPE_EMMC0) || (host->id == TCC_MMC_TYPE_EMMC1)) {
        if (strstr(saved_command_line, "androidboot.emmc_hs_mode=1")) {
//            printk("eMMC High speed MODE set!\n\n");
            printk("eMMC Clock speed set!\n\n");            
//            mmc->caps |= MMC_CAP_BUS_WIDTH_TEST | MMC_CAP_CMD23;
...
// FIXME: Should move?
	...
	host->early_suspend.suspend = tcc_mmc_early_suspend;
	host->early_suspend.resume	= tcc_mmc_late_resume;
	...
	if(host->cd_irq < 0) {
        ...
        /* [downer] M130821 IRQ detect change workqueue*/        
        if (host->id == TCC_MMC_TYPE_SD) { /* Only SD!!!!! */
            sd30_detect_wq = (struct sd30_detect_workqueue *)kzalloc(sizeof(struct sd30_detect_workqueue), GFP_KERNEL);
            if (sd30_detect_wq == NULL) {
                dev_err(&pdev->dev, "failed to allocate sd30_detect_wq\n");     
                goto error;       
            }
            
            INIT_WORK(&sd30_detect_wq->wq, sd30_irq_change_handler);
                
            host->oldstat = 0;//-1;

    		if (host->pdata->cd_int_config) {
    			if(host->pdata->cd_int_config(host->dev, host->id, host->cd_irq) == 0) 
    				ret = request_irq(host->cd_irq, tcc_mmc_cd_irq, IRQF_DISABLED | IRQ_TYPE_EDGE_FALLING, "sd30_detect", host);
    		}
    		...
    		if (ret) {
                ...
                if (host->id == TCC_MMC_TYPE_SD) {
                    g_set_sd30_detect_irq = 1;
                    sd30_detect_wq->irq = pdata->cd_irq_num;
                    sd30_detect_wq->host = host;
                }
            }
            // [downer] M140104 comment for duplicate check
    		//tcc_mmc_check_status(host); 
        }
	}
    ...
    if (host->id == TCC_MMC_TYPE_SD) {
#ifdef USE_WAKELOCK        
        wake_lock_init(&sd_wakelock, WAKE_LOCK_SUSPEND, "sd_wakelock");    /* [downer] A140121 */
#endif
        if (host->pdata->switch_voltage)
            host->pdata->switch_voltage(host->dev, host->id, SD30_ENABLE); /* SD I/O voltage disable */
    }
