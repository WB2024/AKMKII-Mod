...
static int mmc_blk_issue_flush(struct mmc_queue *mq, struct request *req)
{
	...
	if (ret == -ENODEV) {
		printk("%s: %s: restart mmc card",
				req->rq_disk->disk_name, __func__);
#if 0
		if (mmc_blk_reset(md, card->host, MMC_BLK_FLUSH))
			printk("%s: %s: fail to restart mmc",
				req->rq_disk->disk_name, __func__);
		else
	...
	if (ret == -ETIMEDOUT) {
		printk("%s: %s: requeue flush request after timeout",
				req->rq_disk->disk_name, __func__);
		...
		printk("%s: %s: notify flush error to upper layers",
				req->rq_disk->disk_name, __func__);
		ret = -EIO;
	}
...
#else  /* [downer] A131125 force set physical card index */
#if (CUR_AK == MODEL_AK500N)    /* [downer] M141006 */
    switch(card->host->index) {
    case 0: // eMMC
        md->name_idx = card->host->index;        
        break;
    default: // SD, WiFi
        md->name_idx = card->host->index + 1;
    }
#else
    md->name_idx = card->host->index;
#endif
...
static const struct mmc_fixup blk_fixups[] =
{
    ...
    MMC_FIXUP("SEM64G", 0x2, 0x100, add_quirk, MMC_QUIRK_INAND_CMD38),
