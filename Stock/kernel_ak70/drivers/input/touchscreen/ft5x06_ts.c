...
int  toucch_backlight_onoff = 1;
int  touch_pressed_flag = 0;
int  touch_suspend_flag = 0;
...
{
#if 1
	gpio_request(GPIO_TSP_PWEN, "GPIO_TSP_PWEN");
	tcc_gpio_config(GPIO_TSP_PWEN,GPIO_FN(0));
	gpio_direction_output(GPIO_TSP_PWEN,1);
#else
...
static void ft5x06_power_off(void)
{
    volatile PGPIO pGPIO = (volatile PGPIO)tcc_p2v(HwGPIO_BASE);
    ...
    pGPIO->GPFDAT.bREG.GP00 = 0; /* [downer] A131125 reset low */
}

void ft5x06_reset_touch(void)
{
	gpio_request(GPIO_TOUCH_RST, "GPIO_TOUCH_RST");
	tcc_gpio_config(GPIO_TOUCH_RST,GPIO_FN(0));
...
E_UPGRADE_ERR_TYPE  fts_ctpm_fw_upgrade(FTS_BYTE* pbt_buf, FTS_DWRD dw_lenth)
{
    ...
    delay_qt_ms(10);  //set delay 10ms for 5606   20140715 파이조정
    ...
    /*********Step 3:check READ-ID***********************/        
     delay_qt_ms(1000);   // set addtional delay for 5606   20140715 파이조정    
    cmd_write(0x90,0x00,0x00,0x00,4);
    byte_read(reg_val,2);
    //if (reg_val[0] == 0x79 && reg_val[1] == 0x3)
    if (reg_val[0] == 0x79 && reg_val[1] == 0x6) // changing 0x03 as 0x06 for 5606  20140715 파이조정 
    {
...
static int ft5x0x_read_data(void)
{
	...
	//event->touch_point = buf[2] & 0x07;
        event->touch_point = buf[2] & 0x0F; //0x07 for 5-point,0x0F for 10-point feature  20140715 파이조정
...
static void ft5x0x_report_value(void)
{
	...
	struct ts_event *event = &data->event;
	int i;
 //printk( "[FTS] ft5x0x_report_value \n");
	for (i  = 0; i < event->touch_point; i++) {
	    if (event->au16_x[i] < SCREEN_MAX_X && event->au16_y[i] < SCREEN_MAX_Y) {
	 	touch_pressed_flag = 1;	// pressed	
		// add 2014-09-15  lcd 켜질때. 터치 좌표 먹는 문제 수정 임의로 (0,0)를 보냄.. 
		if (toucch_backlight_onoff == 0)
		{
			event->au16_x[i] = SCREEN_MAX_X + 10;	// over head value
			event->au16_y[i] = SCREEN_MAX_Y + 10;   // over head value
			//toucch_backlight_onoff = 1;
		}
//printk( "[FTS] ft5x0x_report_value %d, %d\n",  event->au16_x[i], event->au16_y[i]);	
            // LCD view area
    ...
    if (event->touch_point == 0) {
        ft5x0x_ts_release();
	if (touch_suspend_flag==0)
		 toucch_backlight_onoff = 1;
	 touch_pressed_flag = 0;		//released 
...
***********************************************************************************************/
//static int 
static int __devinit ft5x0x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    tcc_gpio_config(TCC_GPB(11), GPIO_INPUT | GPIO_FN0); // TSP_INT

    tcc_gpio_config_ext_intr(client->irq, EXTINT_GPIOB_11);
...
//	free_irq(client->irq, ft5x0x_ts);
    ...
    destroy_workqueue(ft5x0x_ts->ts_workqueue);
    i2c_set_clientdata(client, NULL); 
//dean test	del_timer(&test_timer);
    return 0;
}
