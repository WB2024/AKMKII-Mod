...
/*
 * [downer] A140107 for dvfs lock
 */
static int g_dvfs_lock = 0;

int dvfs_lock_highspeed(void) 
{
    return g_dvfs_lock;
}
...
static ssize_t store_dvfs_lock(struct cpufreq_policy *policy,
					const char *buf, size_t count)
{
    ...
    switch(enable) {
    case 0:
        printk("dvfs_lock disable\n");
        g_dvfs_lock = 0;

        break;
    case 1:
        printk("dvfs_lock enable\n");
        g_dvfs_lock = 1;
    }
...
static ssize_t show_dvfs_lock(struct cpufreq_policy *policy, char *buf)
{
    if (g_dvfs_lock) 
        return sprintf(buf, "dvfs lock!\n");
    else
        return sprintf(buf, "dvfs unlock!\n");        
}
/**
...
cpufreq_freq_attr_rw(dvfs_lock);
