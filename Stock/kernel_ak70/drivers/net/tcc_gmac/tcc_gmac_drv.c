...
static void tcc_gmac_add_phase_rxclk(unsigned long ioaddr, int isrx, int on)
{
	u32 reg = 0;

	if (isrx == MAC_RX) {
		reg = readl(ioaddr + MAC_DLY3);
		reg |= (on << MAC_DLY_RXCLKIINV);
		
		printk("%s : adding phase 180 degree to RXCLK : %08x\n", __func__, reg);
		writel(reg, ioaddr + MAC_DLY3);
	} else {
		reg = readl(ioaddr + MAC_DLY0);
		reg |= (on << MAC_DLY_TXCLKOINV);
		
		printk("%s : adding phase 180 degree to TXCLK : %08x\n", __func__, reg);
		writel(reg, ioaddr + MAC_DLY0);
	}
}

static void tcc_gmac_add_delay_clock(unsigned long ioaddr, int isrx, int val)
{
	u32 reg = 0;

	if (isrx == MAC_RX) {
		reg = readl(ioaddr + MAC_DLY3);
		reg |= (val << MAC_DLY_RXCLKIDLY);
		
		printk("%s : adding delay %d * 0.1 ns to RXCLK : %08x\n", __func__, val, reg);
		writel(reg, ioaddr + MAC_DLY3);
	} else {
		reg = readl(ioaddr + MAC_DLY0);
		reg |= (val << MAC_DLY_TXCLKODLY);
		
		printk("%s : adding delay %d * 0.1 ns to TXCLK : %08x\n", __func__, val, reg);
		writel(reg, ioaddr + MAC_DLY0);
	}
}

static void disable_clk125(struct net_device *dev) 
{
	...
	reg = bus->read(bus, phydev->addr, 0x10); /* PHYCR */

	reg |= 0x00000010;

	bus->write(bus, phydev->addr, 0x10, (u16)reg);

	reg = bus->read(bus, phydev->addr, 0x10); /* PHYCR */
	printk("Addr : %d PHYCR : %08x\n", phydev->addr, reg);
}
...
//	bus->phy_mask = 0xfffffffc;
	bus->phy_mask = 0x00000001;
...
static int tcc_gmac_open(struct net_device *dev)
{
	...
	tcc_gmac_add_delay_clock(ioaddr, MAC_RX, 15);
	tcc_gmac_add_delay_clock(ioaddr, MAC_TX, 31);
...
static int tcc_gmac_probe(struct platform_device *pdev)
{
	...
	if(wifi_mac != NULL)
	{
		for(i=0;i<MAC_ADDRESS_LEN;i++)
		{
			MAC_addr[i]=(char_to_num(wifi_mac[i*3+1]) &0xF)|(char_to_num(wifi_mac[i*3])&0xF)<<4;
			pr_debug("MAC_addr[%d] %x mac %x %x \n",i,MAC_addr[i],wifi_mac[i*3],wifi_mac[i*3+1]);
		}
		
		/* index from 'MAC_ADDRESS_LEN - 4' to 'MAC_ADDRESS_LEN - 6' are OUI so should not be modified */
		if (MAC_addr[MAC_ADDRESS_LEN - 1] == 0xff) {
			if (MAC_addr[MAC_ADDRESS_LEN - 2] == 0xff) {
				if (MAC_addr[MAC_ADDRESS_LEN - 3] == 0xff) {
					MAC_addr[MAC_ADDRESS_LEN - 1] = 0x00;
					MAC_addr[MAC_ADDRESS_LEN - 2] = 0x00;
					MAC_addr[MAC_ADDRESS_LEN - 3] = 0x00;
				} else {
					MAC_addr[MAC_ADDRESS_LEN - 1] = 0x00;
					MAC_addr[MAC_ADDRESS_LEN - 2] = 0x00;
					MAC_addr[MAC_ADDRESS_LEN - 3]++;
				}
			} else {
				MAC_addr[MAC_ADDRESS_LEN - 1] = 0x00;
				MAC_addr[MAC_ADDRESS_LEN - 2]++;
			}
		} else 
			MAC_addr[MAC_ADDRESS_LEN - 1]++;
