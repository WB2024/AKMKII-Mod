...
char country_code[MOD_PARAM_PATHLEN];
...
module_param_string(country_code, country_code, MOD_PARAM_PATHLEN, 0);
	...
	if (IS_ERR(fp)) {
		...
		if(strlen(country_code) != 0) {
			if(strcmp(country_code, "US") == 0) {
				strncpy(cspec.country_abbrev, "US", sizeof("US"));
				strncpy(cspec.ccode, "US", sizeof("US"));
				cspec.rev = 46;
			} else if(strcmp(country_code, "JP") == 0) {
				strncpy(cspec.country_abbrev, "JP", sizeof("JP"));
				strncpy(cspec.ccode, "JP", sizeof("JP"));
				cspec.rev = 5;
			} else if(strcmp(country_code, "KR") == 0) {
				strncpy(cspec.country_abbrev, "KR", sizeof("KR"));
				strncpy(cspec.ccode, "KR", sizeof("KR"));
				cspec.rev = 24;
			}
		} else {
			strncpy(cspec.country_abbrev, "KR", sizeof("KR"));
		...
		printk("%s: %s open failed. default set country ccode:%s country_abrev:%s rev:%d\n",
				__FUNCTION__,
				filepath,
				cspec.ccode,
		...
		if (kernel_read(fp, 0, buffer, sizeof(buffer))) {
			...
			if (country_offset != 0) {
				strncpy(ccode, buffer, country_offset);
				...
				if (!ret) {
					DHD_ERROR(("%s: get country ccode:%s country_abrev:%s rev:%d  \n",
						...
						cspec.country_abbrev, 
						cspec.rev));
					if ((strncmp(ccode, cspec.ccode, WLC_CNTRY_BUF_SZ) != 0) 
							|| (cspec.rev != country_rev)) {
						strncpy(cspec.country_abbrev, ccode, country_code_size);
						strncpy(cspec.ccode, ccode, country_code_size);
						cspec.rev = country_rev;
