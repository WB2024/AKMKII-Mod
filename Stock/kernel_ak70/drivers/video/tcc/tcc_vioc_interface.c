...
static int  first_32bit=0; //iriver-ysyim

/* [downer] A140108 lcd state */
...
extern void backlight_off(void); //iriver-ysyim
extern TCC_OUTPUT_TYPE	Output_SelectMode;

extern int get_sleep_time(void); //iriver-ysyim
...
#if defined(CONFIG_LCD_S6E63M0X01) || defined(CONFIG_LCD_AK380DISP)
...
#define LCDC_FB_WIDTH	480
#define LCDC_FB_HEIGHT	854
#else
#define LCDC_FB_WIDTH	320
#define LCDC_FB_HEIGHT	240
#endif
...
void tca_fb_activate_var(struct tccfb_info *fbi,  struct fb_var_screeninfo *var)
{
	...
	if(fbi->fb->var.bits_per_pixel == 32) 	{
		...
		//iriver-ysyim:  처음 16bit ->32 bit로 변결될때 한번만 video buffer clear시킴 
		if(first_32bit==0){
...
#if (CUR_AK == MODEL_AK500N) //jimi.141014.to avoid last 4 lines
	if(img_width > LCDC_FB_HEIGHT-4)
		img_width = LCDC_FB_HEIGHT-4;

	if(img_height > LCDC_FB_HEIGHT-4)
		img_height = LCDC_FB_HEIGHT-4;
#endif

#if defined(CONFIG_LCD_AKJR2DISP) //2016.2.26 ysyim: 480x854-> 480x800 
/*
	if(img_width > LCDC_FB_HEIGHT-54)
		img_width = LCDC_FB_HEIGHT-54;
	
	if(img_height > LCDC_FB_HEIGHT-54)
		img_height = LCDC_FB_HEIGHT-54;
*/		
#endif
	...
	switch(imgch)
	{
		case 0:
			...
			//VIOC_WMIX_SetPosition(pWMIXBase, imgch, 0, 54); //2016.2.26 ysyim: LCD y position is modified.
			VIOC_WMIX_SetPosition(pWMIXBase, imgch, 0, 0); //2016.2.26 ysyim: LCD y position is modified.
			#else
...
#ifdef CONFIG_HAS_EARLYSUSPEND
extern int get_keylock_state(void);
extern int ak_wheel_lock_mode(int mode);

#ifdef USE_LCD_OFF_WAKELOCK
void ak_set_lcd_off_wakelock(void)
{
	// refer to Tcc_vioc_interface.c : default 180sec
	// sleep delay time control
	// sleep after 10 sec. ex) #echo 10 > /sys/devices/platform/astell_kern_drv/sleep_time
	int sleep_time = get_sleep_time(); 
		
  wake_lock_timeout(&lcd_wakelock, sleep_time * HZ); /* [downer] A140211 3 minutes wakelock */
	printk("\x1b[1;31mgo_to_sleep_after: %dsec \x1b[0m\n",sleep_time);               
}
EXPORT_SYMBOL(ak_set_lcd_off_wakelock);
#endif
...
void tca_fb_earlier_suspend(struct early_suspend *h)
{
    ...
    /* [downer] A140211 LCD off 시 global keylock 모드이면 휠 키 잠금. */
    if (get_keylock_state())
        ak_wheel_lock_mode(1);
}
...
extern int  toucch_backlight_onoff;
extern int  touch_pressed_flag;
extern void ft5x06_reset_touch(void);
#endif

void tca_fb_later_resume(struct early_suspend *h)
{
#ifdef USE_LCD_OFF_WAKELOCK
    printk(KERN_INFO "lcd-on wake unlock --\n");
    wake_unlock(&lcd_wakelock);
#endif

    /* [downer] A140211 LCD on 시 global keylock 모드이면 휠 키 unlock */
    if (get_keylock_state())
        ak_wheel_lock_mode(0);

#if (CUR_AK == MODEL_AK500N)
	if (touch_pressed_flag == 0)
	{
		toucch_backlight_onoff = 1;
		// emergency touch reset !!!
		ft5x06_reset_touch();
	}	
#endif
}
...
{
    if (g_lcd_state) {     /* [downer] A140421 for power off or reset */
    	printk("%s\n", __func__);
			
		backlight_off(); //iriver-ysyim: refert to lcd_ak380.c
...
int fb_hibernation_logo(void)
{
	...
	if(do_hibernation ){
		...
		{		
			output_ret = TCC_OUTPUT_FB_Update_External(LCDC_FB_WIDTH, LCDC_FB_HEIGHT, LCDC_FB_FORMAT, lcdc_logo.base , Output_SelectMode);
