...
int tcc_video_lastframe_move(struct tcc_lcdc_image_update* inFframeInfo, WMIXER_INFO_TYPE* WmixerInfo, unsigned int target_addr, unsigned int target_format)
{
	...
	ret = tccxxx_wmixer_ioctl((struct file *)&lastframe_wm_flip, TCC_WMIXER_IOCTRL_KERNEL, WmixerInfo);
...
#ifdef SCREEN_HEIGHT_FORCE_800	//defined in astell_kern.h 
	screen_height = 800;	//iriver ysyim 16.04.25 : only ak70
#else
