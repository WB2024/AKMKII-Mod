...
static int __devinit s6e63m0_bl_probe(struct spi_device *spi)
{
	int ret = 0;
	...
	printk("s6e63m0 panel driver has been probed.\n");
...
static int __devexit s6e63m0_bl_remove(struct spi_device *spi)
{
...
static int s6e63m0_bl_suspend(struct spi_device *spi, pm_message_t mesg)
{
	int ret = 0;
	struct s6e63m0 *lcd = dev_get_drvdata(&spi->dev);
...
static int s6e63m0_bl_resume(struct spi_device *spi)
{
	int ret = 0;
	struct s6e63m0 *lcd = dev_get_drvdata(&spi->dev);
...
#else
#define s6e63m0_bl_suspend		NULL
#define s6e63m0_bl_resume		NULL
#endif

/* Power down all displays on reboot, poweroff or halt. */
static void s6e63m0_bl_shutdown(struct spi_device *spi)
{
	struct s6e63m0 *lcd = dev_get_drvdata(&spi->dev);
...
static struct spi_driver s6e63m0_bl_driver = {
	.driver = {
		.name	= "s6e63m0_bl",
		.bus	= &spi_bus_type,
		.owner	= THIS_MODULE,
	},
	.probe		= s6e63m0_bl_probe,
	.remove		= __devexit_p(s6e63m0_bl_remove),
	.shutdown	= s6e63m0_bl_shutdown,
	.suspend	= s6e63m0_bl_suspend,
	.resume		= s6e63m0_bl_resume,
};

static int __init s6e63m0_bl_init(void)
{
    printk("%s\n", __func__);
    
	return spi_register_driver(&s6e63m0_bl_driver);
}

static void __exit s6e63m0_bl_exit(void)
{
	spi_unregister_driver(&s6e63m0_bl_driver);
}

//module_init(s6e63m0_bl_init);
subsys_initcall(s6e63m0_bl_init);
module_exit(s6e63m0_bl_exit);
