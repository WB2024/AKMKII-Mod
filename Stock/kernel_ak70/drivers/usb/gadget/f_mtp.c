...
#ifdef _USE_MTP_DELAY_
#define MTP_MS_DELAY					1
#endif // #ifdef _USE_MTP_DELAY_
//#define DMA_MODE

#define MTP_BULK_BUFFER_SIZE	81920//16384	
...
#define TX_REQ_MAX 16//8//4
#define RX_REQ_MAX 16//8//2
#define INTR_REQ_MAX 20//10//5
...
void mtp_transfer_delay(void)
{
	if(mtp_get_play_status()==MPS_OVER_96) {
		msleep_interruptible(MTP_MS_DELAY);
	}
}	
#endif
...
//			read_req->length = (count > MTP_BULK_BUFFER_SIZE ? MTP_BULK_BUFFER_SIZE : count);
...
static int g_lock_flag = 0;

static void mtp_force_unlock(void)
{
    if (g_lock_flag) {
        printk("%s\n", __func__);
