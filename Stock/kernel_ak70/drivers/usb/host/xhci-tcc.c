...
extern void ak_otg_disconnect_finish(void); /* [downer] A160516 */
...
static struct wake_lock xhci_wakelock;
...
static int __devinit xhci_tcc_drv_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_REGULATOR    
    dwc_usb_regulator_enable(); /* [downer] A150710 */
#endif
    
	tcc_init_usb3host(USB3_SPEED_MODE);

    wake_lock_init(&xhci_wakelock, WAKE_LOCK_SUSPEND, "xhci_wakelock"); /* [downer] A150331 */
    ...
    *((struct xhci_hcd **) xhci->shared_hcd->hcd_priv) = xhci;
    ...
    wake_lock(&xhci_wakelock);
    printk(KERN_INFO "%s : xhci wake lock ++\n", __func__);
...
static void xhci_tcc_remove(struct platform_device *pdev)
{
...
#ifdef CONFIG_REGULATOR    
    dwc_usb_regulator_disable(); /* [downer] A150710 */
#endif

    sys_external_otg_state(OTG_UNLOAD); /* [downer] A150831 for event change */
    wake_lock_destroy(&xhci_wakelock);
    ...
    sys_external_otg_state(OTG_DISCONNECT); /* [downer] A150831 for event change */
	ak_otg_disconnect_finish();
...
#if (CUR_AK != MODEL_AK500N && CUR_AK != MODEL_AK380  && CUR_AK != MODEL_AK380S && CUR_AK != MODEL_AK320 && CUR_AK != MODEL_AK300)
