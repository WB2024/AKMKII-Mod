...
int tcc8920_mmc_get_ro(struct device *dev, int id)
{
...
static void tcc8920_mmc_port_setup(void)
{
	...
	if(system_rev < 0x1005)		// for TCC8920 EVM Board
	{
		...
		mmc_ports[TCC_MMC_TYPE_SD3_0].wp	= TCC_MMC_PORT_NULL;
		...
		mmc_ports[TCC_MMC_TYPE_SD].wp	= TCC_MMC_PORT_NULL;
		...
		mmc_ports[TCC_MMC_TYPE_EMMC].wp	= TCC_MMC_PORT_NULL;
