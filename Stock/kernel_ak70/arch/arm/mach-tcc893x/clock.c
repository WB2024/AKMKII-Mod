...
static int pclk_set_rate(struct clk *clk, unsigned long rate)
{
    ...
    local_irq_restore(flags);
    //dbg("%s: clkname:%s, old_rate:%ld, new_rate:%ld(%ld)\n", __func__, clk->name, clk->rate, rate, clk->real_rate);
    //printk("%s: clkname:%s, old_rate:%ld, new_rate:%ld(%ld)\n", __func__, clk->name, clk->rate, rate, clk->real_rate);
    return 0;
}
...
{
    if (!strcmp(clk->name, "usb_otg")) {
        //printk("%s(%s) clk->usecount: %d\n", __func__, clk->name, clk->usecount);
        clk->usecount = 1;  /* [downer] A130813 Only USB_OTG force disable */
    }
