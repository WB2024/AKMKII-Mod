...
//#define CHECK_GPIO_EXT_INT



#ifdef CHECK_GPIO_EXT_INT  /* 2013-7-12 jhlim*/

#define MAX_EXT_INT (INT_EINT11 - INT_EINT0)

static int g_ext_int_alloc[MAX_EXT_INT];
...
int tcc_gpio_config_ext_intr(unsigned intr, unsigned source) /* intr: irq num, source: external interrupt source */
{
	...
	#ifdef CHECK_GPIO_EXT_INT
	int intr_index = intr - INT_EINT0;
	if(intr_index < 0 || intr_index > MAX_EXT_INT) {
		printk(KERN_ERR "EXT INT INDEX ERROR !!!: %d \n",intr_index);
		return 0;
	}
	
	if(g_ext_int_alloc [ intr_index] > 0) {
		printk(KERN_ERR "EXT INT ALLOC ALREADY : %d \n",intr_index);
		return 0;
	}
	#endif
...
#ifdef CHECK_GPIO_EXT_INT
	g_ext_int_alloc[intr_index] = 1;		
#endif


	#ifdef DISPLAY_GPIO_EXT_INT_ALLOC_LIST

	{
		int i;

		for(i=0;i<MAX_EXT_INT;i++) {
			printk(KERN_ERR "EXT_INT : %d %d\n",i,g_ext_int_alloc[i]);
		}
	}
	#endif
...
int tcc893x_direct_gpio_direction_input(unsigned gpio_group, unsigned offset)
{
	gpioregs *reg = tcc893x_gpio_to_reg(gpio_group);
	unsigned long flags;
	
	//printk("tcc893x_direct_gpio_direction_input: reg:0x%x,offset:%d,&reg->out_en:0x%x,&reg->out_en:0x%x\n",reg,offset,&reg->out_en,&reg->out_en);
...
EXPORT_SYMBOL(tcc893x_direct_gpio_direction_input);

int tcc893x_direct_gpio_direction_output(unsigned gpio_group, unsigned offset, int value)
{
	gpioregs *reg = tcc893x_gpio_to_reg(gpio_group);
	unsigned long flags;
...
EXPORT_SYMBOL(tcc893x_direct_gpio_direction_output);
