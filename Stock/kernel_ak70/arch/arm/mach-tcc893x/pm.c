...
//	((PPMU)HwPMU_BASE)->PMU_ISOL.nREG = 0x00000BDF;
	((PPMU)HwPMU_BASE)->PMU_ISOL.nREG = 0x00000BDD;
...
//Bruce, wake-up source should be surely set here !!!!
	...
	{
		//set wake-up polarity
		((PPMU)HwPMU_BASE)->PMU_WKPOL0.bREG.GPIO_G14 = 0; //power key - Active High 
		((PPMU)HwPMU_BASE)->PMU_WKUP0.bREG.GPIO_G14 = 1; //power key

		/* wakeup by bluetooth */
		((PPMU)HwPMU_BASE)->PMU_WKPOL1.bREG.GPIO_E24 = 0; 
		((PPMU)HwPMU_BASE)->PMU_WKUP1.bREG.GPIO_E24 = 1; 

		/* wakeup by low battery */
		((PPMU)HwPMU_BASE)->PMU_WKPOL0.bREG.GPIO_G05 = 1; // 2013.12.16 iriver-ysyim: Active Low
		((PPMU)HwPMU_BASE)->PMU_WKUP0.bREG.GPIO_G05 = 1; 

        if (*(volatile unsigned long *)(SRAM_STACK_ADDR+0x4) == 1) { // usb state
            /* wakeup by USB/Adapter device */
            ((PPMU)HwPMU_BASE)->PMU_WKPOL1.bREG.GPIO_E28 = 0; /* CHG_DET */
            ((PPMU)HwPMU_BASE)->PMU_WKUP1.bREG.GPIO_E28 = 1; 
        }
        else {
            /* wakeup by USB device */
            if (*(volatile unsigned long *)(SRAM_STACK_ADDR+0x8) == 1) { // pc-fi state
                ((PPMU)HwPMU_BASE)->PMU_WKPOL1.bREG.GPIO_E28 = 0; /* CHG_DET */
                ((PPMU)HwPMU_BASE)->PMU_WKUP1.bREG.GPIO_E28 = 1; 
            }
            else {
                ((PPMU)HwPMU_BASE)->PMU_WKPOL1.bREG.GPIO_E28 = 1; /* CHG_DET */
                ((PPMU)HwPMU_BASE)->PMU_WKUP1.bREG.GPIO_E28 = 1; 
            }
        }
		
        /* wakeup by event key */
		((PPMU)HwPMU_BASE)->PMU_WKPOL0.bREG.GPIO_G10 = 1; /* KEY_PLAY_STOP */
		((PPMU)HwPMU_BASE)->PMU_WKUP0.bREG.GPIO_G10 = 1; 
#if 0
		((PPMU)HwPMU_BASE)->PMU_WKPOL0.bREG.GPIO_G11 = 1; /* KEY_BACK */
		((PPMU)HwPMU_BASE)->PMU_WKUP0.bREG.GPIO_G11 = 1;

		((PPMU)HwPMU_BASE)->PMU_WKPOL0.bREG.GPIO_G13 = 1; /* KEY_NEXT */
		((PPMU)HwPMU_BASE)->PMU_WKUP0.bREG.GPIO_G13 = 1;
#endif
        
#if defined(CONFIG_MMC_TCC_SDHC)	// Wakeup for SD Insert->Remove in Suspend.
		if (*(volatile unsigned long *)(SRAM_STACK_ADDR+0xC) == 1)	{	// SD Insert -> Remove in suspend : Active High
            ((PPMU)HwPMU_BASE)->PMU_WKPOL0.bREG.GPIO_G12 = 0; /* SD remove */
			((PPMU)HwPMU_BASE)->PMU_WKUP0.bREG.GPIO_G12 = 1;	
        }
#endif
	}
#endif
...
// clear wake-up
	...
	((PPMU)HwPMU_BASE)->PMU_WKCLR0.nREG = 0xFFFFFFFF & ~(Hw23 | Hw16 | Hw19 | Hw21);
	((PPMU)HwPMU_BASE)->PMU_WKCLR1.nREG = 0xFFFFFFFF & ~(Hw24 | Hw28);
#endif
...
// BUS Power On
	...
	//IP isolation on
	((PPMU)HwPMU_BASE)->PMU_ISOL.nREG = 0x00000899;
...
static void shutdown_mode(void)
{
	...
	*(volatile unsigned int *)(L2CACHE_BASE+L2X0_CTRL) = 0; //cache off
	...
	//__memcpy((PPMU)tcc_p2v(HwPMU_BASE), &RegRepo.pmu, sizeof(PMU));
	{
		...
		((PPMU)tcc_p2v(HwPMU_BASE))->PMU_ISOL.nREG = 0x00000899; // 0x00000000 [downer] M131011
	}
#endif
...
// set board information
	*(volatile unsigned long *)SRAM_STACK_ADDR = system_rev;
    ...
    /* [downer] M131205 */
	*(volatile unsigned long *)(SRAM_STACK_ADDR+0x4) = get_usb_plug_state();
	*(volatile unsigned long *)(SRAM_STACK_ADDR+0x8) = get_pcfi_suspend_enable();
#if defined(CONFIG_MMC_TCC_SDHC)    
	*(volatile unsigned long *)(SRAM_STACK_ADDR+0xC) = tcc893x_sd_card_detect();
#endif
...
{
	printk("tcc_pm_power_off\n");

	#if 0 //def REDUCE_POP_2th
...
#elif defined(CONFIG_REGULATOR_ACT8840)
	extern void act8840_power_off(void);
	act8840_power_off();
#endif
	...
	if (code == SYS_RESTART) {
		if (cmd) {
			if (!strcmp(cmd, "bootloader")) {
				...
				restart_reason = 3; //2013.10.23-irier.ysyim: 0->3 (adb reboot) 
			}
		} else {
			restart_reason = 3; //2013.10.23-irier.ysyim: 0->3 (reboot)
		}
	}
	return NOTIFY_DONE;
}
