...
//        msleep(1);
		...
		msleep_interruptible(time_delay++);
...
/*----------------------------------------------------------------------
 * device     : SPI GPIO
 * description: SPI function based GPIO
 *----------------------------------------------------------------------*/
#if defined(CONFIG_MTD_M25P80_XMOS)
static struct spi_gpio_platform_data ak201_spi_bitbang_platform_data = {
		.sck 	= GPIO_XMOS_SCLK,
		.mosi	= GPIO_XMOS_SDO,
		.miso	= GPIO_XMOS_SDI,
		.num_chipselect		= 1
...
struct platform_device spi_bitbang_device = {
	.name 		= "spi_gpio",
	.id			= 0,
	.dev		= {
		.platform_data = &ak201_spi_bitbang_platform_data
	}
};
#endif
...
static struct spi_gpio_platform_data audiodsp_spi_bitbang_platform_data = {
	.sck 	= GPIO_DSP_SCK,
	.mosi	= GPIO_DSP_MOSI,
	.miso	= GPIO_DSP_MISO,
	.num_chipselect		= 1,
...
struct platform_device spi_bitbang1_device = {
	.name 		= "spi_cs48l1x",
	.id			= 2,
	.resource	= spi2_resources,
	.num_resources	= ARRAY_SIZE(spi2_resources),
	.dev		= {
		.platform_data = &audiodsp_spi_bitbang_platform_data
	}
};
#endif
