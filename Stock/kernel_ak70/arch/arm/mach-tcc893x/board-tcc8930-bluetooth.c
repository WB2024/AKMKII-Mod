...
#define BT_REG_GPIO			GPIO_BT_REG_ON
#define BT_WAKE_GPIO 		GPIO_BT_DEV_WAKE
...
static int tcc_bt_rfkill_set_power(void *data, bool blocked)
{
	...
	if (!blocked && (bt_enable_flag == 0)) {
		  ...
		  g_bt_enabled = 1;
		  msleep(10);
        if (machine_is_tcc8800() || machine_is_tcc893x() || machine_is_m805_893x()) {      // #elif defined (CONFIG_MACH_TCC9300)
			...
			printk("tcc_uart: BT power ON\n");

			gpio_set_value(BT_WAKE_GPIO, 1);		// BT_DEV_WAKE
			gpio_set_value(BT_REG_GPIO, 0);		// BT_REG_ON
			msleep(500);
			...
			tcc_gpio_config(BT_HOST_WAKE_GPIO, GPIO_INPUT | GPIO_FN0);		// BT_HOST_WAKE
#endif	
		}
		else if (machine_is_m801_88() || machine_is_m803())
		{
		...
		if (machine_is_tcc8800() || machine_is_tcc893x() || machine_is_m805_893x()) {      // #elif defined (CONFIG_MACH_TCC9300)
			...
			tcc_gpio_config(BT_HOST_WAKE_GPIO, GPIO_INPUT | GPIO_FN0 | GPIO_PULL_DISABLE);		// BT_HOST_WAKE
#endif			
		}
		else if (machine_is_m801_88() || machine_is_m803())
		{
...
static int __init tcc_bluetooth_init(void)
{
	...
	// BT GPIO setup
	tcc_gpio_config(BT_HOST_WAKE_GPIO, GPIO_INPUT | GPIO_FN0);		// BT_HOST_WAKE
	tcc_gpio_config(BT_WAKE_GPIO, GPIO_OUTPUT | GPIO_FN0);	// BT_DEV_WAKE
	tcc_gpio_config(BT_REG_GPIO, GPIO_OUTPUT | GPIO_FN0);	// BT_REG_ON
