...
#if defined(CONFIG_SPI_CS48L1X)
#include <linux/spi/spi_cs48l1x.h>
#endif

#if defined(CONFIG_SND_SOC_AK5572)
#include <sound/ak5572_pdata.h>
#endif

//2013.11.20 iriver-ysyim
...
#if defined(CONFIG_MTD_M25P80_XMOS)
static struct flash_platform_data ak201_spi_flash_platform_data = {
		.name = "XMOS-boot",
		.type = "m25p40"
};

static struct spi_board_info tcc8930_spi0_mtd_board_info[] = {
	{
		.modalias = "m25p80",
		.bus_num = 0,					// spi channel 0
		.chip_select = 0,
		/* you can modify the following member variables */
		.platform_data = &ak201_spi_flash_platform_data,
		.controller_data = GPIO_XMOS_SS,	// Chip Select pin
...
#if defined(CONFIG_SPI_CS48L1X)
static struct spi_gpio_ext_data tcc8930_spi_cs48l1x_controller_data = {
	.cs = GPIO_DSP_CS,
	.busy = GPIO_DSP_BUSY_HS0,
	.read = 0x81,
	.write = 0x80,
};
#endif

static struct spi_board_info tcc8930_spi2_board_info[] = {
	{
		.modalias = "spidev",
		.bus_num = 2,					// spi channel 2
		.chip_select = 0,
		/* you can modify the following member variables */
#if defined(CONFIG_SPI_CS48L1X)
		.controller_data = &tcc8930_spi_cs48l1x_controller_data,	// Chip Select pin
		.irq = GPIO_DSP_INT_HS1,
#endif
//		.max_speed_hz = 1*1000*1000,	// default 2Mhz
		.max_speed_hz = 50*1000,	// default 2Mhz
		.mode = 0						// default 0, you can choose [SPI_CPOL|SPI_CPHA|SPI_CS_HIGH|SPI_LSB_FIRST]
	},
};
...
static struct pcf857x_platform_data pcf8575_data = {	//2015.4.15:ysyim
    .gpio_base  = NR_BUILTIN_GPIO,
};
#endif

#if defined(CONFIG_SND_SOC_AK5572)
static struct ak5572_platform_data ak5572_pdata = {
.pdn_gpio = EXT_GPIO_P00, //assign of gpo pin for PDN
};
#endif
...
static struct i2c_board_info __initdata i2c_devices0[] = {
	...
	#if 0 //def CONFIG_SND_SOC_TCC_BOARD_WM8804
	{
	...
	{
		I2C_BOARD_INFO("CS4398_1", 0b1001111), 
		.platform_data = NULL,
	},
	{
		I2C_BOARD_INFO("CS4398_2", 0b1001100),
		.platform_data = NULL,
	},
		#if (CUR_AK == MODEL_AK500N)
		,
		{
			I2C_BOARD_INFO("CS3318", 0b1000000),
			.platform_data = NULL,
		},
		{
			I2C_BOARD_INFO("CS8416", 0x26>>1),
			.platform_data = NULL,
		},
		{
			I2C_BOARD_INFO("CS8406", 0x2E>>1),
			.platform_data = NULL,
		},
		#endif
	...
	{
		I2C_BOARD_INFO("AK4490_1", 0b0010001),
		.platform_data = NULL,
	},
	{
		#ifdef AK_AUDIO_MODULE_USED
		I2C_BOARD_INFO("AK4490_2", 0b0010011), // clock generator와 충돌 피하기 위해 임시로 변경
		#else
		I2C_BOARD_INFO("AK4490_2", 0b0010000),.
		#endif
		.platform_data = NULL,
	},
	...
	{
		I2C_BOARD_INFO("ES9018_1", 0x90>>1), 
		.platform_data = NULL,
	},
	{
		I2C_BOARD_INFO("ES9018_2", 0x92>>1),
		.platform_data = NULL,
	},		
	{
		I2C_BOARD_INFO("CS3318", 0b1000000),
...
#if defined(CONFIG_GPIO_PCF857X)
	{
		I2C_BOARD_INFO("pcf8575", 0b0100001), //2015.4.15:ysyim
		.platform_data = &pcf8575_data,
	},
#endif	
	...
	//J151021+
	{
		I2C_BOARD_INFO("SA9227", 0b1101010), //0xd4
		.platform_data = NULL,
	},
	{
		I2C_BOARD_INFO("ak5572", 0b0010011), //0x10
		.platform_data = &ak5572_pdata,
	},
	{
		I2C_BOARD_INFO("MAX14662_0", 0b1001111),
		.platform_data = NULL,
	},			
	{
		I2C_BOARD_INFO("MAX14662_1", 0b1001100),
		.platform_data = NULL,
	},			
	{
		I2C_BOARD_INFO("MAX14662_2", 0b1001110),
		.platform_data = NULL,
	},			
	//J151021-
	{
		I2C_BOARD_INFO("PGA2505_L", 0b0101100),
		.platform_data = NULL,
	},	
	{
		I2C_BOARD_INFO("PGA2505_R", 0b0101110),
		.platform_data = NULL,
	},	
#endif
#if(AUDIO_CLOCK == CLK_AK8157A)
	{
		I2C_BOARD_INFO("AK8157A", 0b0010011),
		.platform_data = NULL,
	},
#endif
};
...
#endif
#if defined(CONFIG_TOUCHSCREEN_IST3000)
	{
		I2C_BOARD_INFO("IST3000", 0xA0>>1),
		.irq = INT_EINT11,
		.platform_data = NULL,
	},
#endif
#if defined(CONFIG_TOUCHSCREEN_FT5X16)
	{
		I2C_BOARD_INFO("ft5x16_ts", 0x70 >> 1),
		.irq = INT_EINT11,
		.platform_data = NULL,
	},
#endif
...
static struct tcc_i2c_platform_data tcc8930_core1_platform_data = {
    ...
    .core_clk_name      = "i2c1",
    .smu_i2c_flag       = 0,
    .i2c_ch_clk_rate[0] = 200,      /* SCL clock rate : 400kHz */
};
#endif
#if defined(CONFIG_I2C_TCC_CORE2)
...
void sata_host_5v_reset(void)
{
	gpio_request(GPIO_HUB_RST, "GPIO_HUB_RST");
	tcc_gpio_config(GPIO_HUB_RST, GPIO_FN(0)); 
	gpio_direction_output(GPIO_HUB_RST,0);

	gpio_request(GPIO_CD_HOST_5V_EN, "GPIO_CD_HOST_5V_EN");
	tcc_gpio_config(GPIO_CD_HOST_5V_EN, GPIO_FN(0)); 
	gpio_direction_output(GPIO_CD_HOST_5V_EN,0);  // force  off -> on.
    
	gpio_request(GPIO_SATA_HOST_5V_EN, "GPIO_SATA_HOST_5V_EN");
	tcc_gpio_config(GPIO_SATA_HOST_5V_EN, GPIO_FN(0)); 
	CPRINT("USB_HOST_5V init start\n");
	gpio_direction_output(GPIO_SATA_HOST_5V_EN,0);	// force  off -> on.
	msleep_interruptible(300); 

	gpio_direction_output(GPIO_HUB_RST,1);

	if( get_ssd_attach() > 1) {
		msleep_interruptible(5000);   // if 4ea ssd attached , delay needs.
	} else {
		msleep_interruptible(500);   // if 4ea ssd attached , delay needs.
	}
	gpio_direction_output(GPIO_SATA_HOST_5V_EN,1);	 
	gpio_direction_output(GPIO_CD_HOST_5V_EN,1);

	gpio_set_value(GPIO_SATA_HOST_5V_EN, 1); 
	gpio_set_value(GPIO_CD_HOST_5V_EN, 1); 

	CPRINT("USB_HOST_5V init end\n");
}
#endif
...
static void __init tcc8930_init_machine(void)
{
	...
	gpio_direction_output(GPIO_USB_ONOFF, 0); // #OE : Active Low
	...
	gpio_direction_output(GPIO_USB20_HOST_EN, 0); // #OE : Active Low
	...
	gpio_request(GPIO_REAR_HOST_5V_EN, "GPIO_REAR_HOST_5V_EN");
	tcc_gpio_config(GPIO_REAR_HOST_5V_EN, GPIO_FN(0)); 
	gpio_direction_output(GPIO_REAR_HOST_5V_EN,1);

	CPRINT("RAID ssd_num:%d raid%d\n",get_ssd_attach(),get_raid_array_type());
...
#if defined(CONFIG_SPI_CS48L1X)
	spi_register_board_info(tcc8930_spi2_board_info, ARRAY_SIZE(tcc8930_spi2_board_info)); //jhlim
#endif

#if defined(CONFIG_MTD_M25P80_XMOS)
	spi_register_board_info(tcc8930_spi0_mtd_board_info, ARRAY_SIZE(tcc8930_spi0_mtd_board_info));
#endif
...
//	tcc_add_nand_device();
	...
	// 2013-9-13  XMOS default power off;
	// Moved to drivers/mtd/devices/xmos.c
