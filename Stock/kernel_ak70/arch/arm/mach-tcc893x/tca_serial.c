...
void tca_serial_gpio_setting(int nFlag_fc, int nPort)
{
	...
	if(uart_port_map_selection[nPort][0] != -1){
		if(nPort == 2) 
		{
			tcc_gpio_config(uart_port_map_selection[nPort][0], uart_port_map_selection[nPort][4] | GPIO_PULLUP); // tx
			tcc_gpio_config(uart_port_map_selection[nPort][1], uart_port_map_selection[nPort][4] | GPIO_PULLUP); // rx
		}
		else
		{
