...
public class KeyboardView extends View implements View.OnClickListener {
    ...
    public KeyboardView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        mBackgroundDimAmount = 0.6f; //a.getFloat(android.R.attr.backgroundDimAmount, 0.5f);
...
//            mKeyboard.resize(w, h);
    ...
    private CharSequence getPreviewText(Key key) {
    	// Add, 170419 yunsuk [Simeji Key]
		if (key.codes[0] >= JapaneseManager.KEYCODE_HIRA_START 
    ...
    private void showKey(final int keyIndex) {
    	// Add, 170508 yunsuk
    	int prevPopupX = mPopupPreviewX;
    	int prevPopupY = mPopupPreviewY;
        ...
        // Add, 170419 yunsuk [
		if (key.modifier == true || key.codes[0] == LatinKeyboard.KEYCODE_EMPTY_BLANK) {
			if (previewPopup.isShowing()) {
		...
		if (key.icon != null) {
			// Original, 170413 yunsuk
			mPreviewText.setCompoundDrawables(null, null, null, key.iconPreview != null ? key.iconPreview : key.icon);
//			mPreviewText.setCompoundDrawables(null, key.iconPreview != null ? key.iconPreview : key.icon, null, null);
		...
		if (!mPreviewCentered) {
			// Original, 170418 yunsuk
			// mPopupPreviewX = key.x - mPreviewText.getPaddingLeft() +  mPaddingLeft;
			mPopupPreviewX = key.x - ((popupWidth - key.width) / 2);

			if (DEBUG_LOG)
        		Log.i(TAG, "key.x : " + key.x + ",mPopupPreviewX : " + mPopupPreviewX);
        ...
        // Add, 170508 yunsuk
		if (previewPopup.isShowing()) {
			if (prevPopupX == mPopupPreviewX && prevPopupY == mPopupPreviewY) {
				// Update
			} else {
				previewPopup.dismiss();
			}
