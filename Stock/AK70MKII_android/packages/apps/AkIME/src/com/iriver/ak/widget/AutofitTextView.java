...
public class AutofitTextView extends TextView {
	...
	private static final int DEFAULT_MIN_TEXT_SIZE = 27; // px
	 ...
	 *            TODO
	 */
	private void refitText(String text, int width, boolean force) {
		if (width > 0) {
...
//				AkLog.d(TAG, "newTextSize: " + newTextSize + " text: " + text);
			}

			if (newTextSize < getTextSize() || force) {
	...
	protected void onTextChanged(final CharSequence text, final int start,
		...
		refitText(text.toString(), this.getWidth(),
				Math.abs(lengthBefore - lengthAfter) > 1);
	}
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		if (w != oldw) {
			refitText(getText().toString(), w, false);
		}
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		refitText(getText().toString(), parentWidth, false);
	}
}