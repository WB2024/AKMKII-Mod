...
// 2017.08.07 iriver-eddy #FIXME #AAC, #DTS, #DIVX
#if defined(_SUPPORT_FFMPEG_FULL_CODEC_)
	#define SUPPOURT_UNKNOWN_FORMAT   1
#else
	#define SUPPOURT_UNKNOWN_FORMAT   0
#endif // #if defined(_SUPPORT_FFMPEG_FULL_CODEC_)
...
// 2016.12.22 iriver-eddy #FFMPEG_FULL_PLAYBACK
#if defined(_SUPPORT_FFMPEG_FULL_CODEC_)
	...
	// 2017.08.07 iriver-eddy #FIXME #AAC, #DTS
    case AV_CODEC_ID_AC3:
    case AV_CODEC_ID_DTS:
#endif // #if defined(_SUPPORT_FFMPEG_FULL_CODEC_)

// 2016.12.05 iriver-eddy #ADD_CODEC, #WMV_PLAYBACK
#if defined(_SUPPORT_WMV_CODEC_)
    case AV_CODEC_ID_WMV1:
    case AV_CODEC_ID_WMV2:
    case AV_CODEC_ID_WMV3:
#endif // #if defined(_SUPPORT_WMV_CODEC_)
    ...
    default:
        ALOGD("### %s() / unsupported codec(%s), but give it a chance ###",
                __FUNCTION__, avcodec_get_name(codec_id));
...
sp<MetaData> FFmpegExtractor::setVideoFormat(AVStream *stream)
{
    ...
    AVCodecContext *avctx __unused = NULL;
...
// 2016.12.05 iriver-eddy #ADD_CODEC, #WMV_PLAYBACK
	...
	default:
        ALOGE("unsupported video codec(id:%d, name:%s), but give it a chance",
...
// 2016.12.22 iriver-eddy #FFMPEG_FULL_PLAYBACK
    ...
    case AV_CODEC_ID_APE:
        ...
        break;
    default:
        ALOGW("unsupported audio codec(id:%d, name:%s), but give it a chance",
                avctx->codec_id, avcodec_get_name(avctx->codec_id));
    ...
    if (meta != NULL) {
		...
		// 2016.11.08 iriver-eddy [24BIT_WMA] #HIFI, #WMA_PLAYBACK
		// if codec is WMA must be set below
		if(avctx->codec_id == AV_CODEC_ID_WMAV1 || avctx->codec_id == AV_CODEC_ID_WMAV2 ||
			avctx->codec_id == AV_CODEC_ID_WMAPRO || avctx->codec_id == AV_CODEC_ID_WMALOSSLESS) {
			meta->setInt32(kKeyBitsPerSample, avctx->bits_per_coded_sample);
			meta->setInt32(kKeyWMABitspersample, avctx->bits_per_coded_sample);
		}

        ALOGD("### %s() / bitrate=%" PRId64 " / sample_rate=%d / channels=%d / "
                "bits_per_coded_sample=%d / block_align=%d / "
                "bits_per_raw_sample=%d / sample_format=%d / bits per sample=%d ###",
                __FUNCTION__, avctx->bit_rate, avctx->sample_rate, avctx->channels,
                avctx->bits_per_coded_sample, avctx->block_align,
                avctx->bits_per_raw_sample, avctx->sample_fmt, bits);
    }
...
int FFmpegExtractor::stream_component_open(int stream_index)
{
    ...
    sp<MetaData> meta = NULL;
    bool supported = false;
    uint32_t type __unused = 0;
    const void *data __unused = NULL;
    size_t size __unused = 0;
    int ret = 0;

    ALOGV("stream_index: %d", stream_index);
    ...
    ALOGV("### %s() / support the codec(%s) disposition(%x) ###", __FUNCTION__, avcodec_get_name(avctx->codec_id), mFormatCtx->streams[stream_index]->disposition);

    unsigned streamType __unused;
...
int FFmpegExtractor::initStreams()
{
    ...
    int pkt_in_play_range __unused = 0;
...
void FFmpegExtractor::stopReaderThread() {
    ...
    if (!mReaderThreadStarted) {
        ALOGV("Reader thread have been stopped");
        mLock.unlock();
        return;
    }
...
{
    enum AVCodecID codec_id __unused = AV_CODEC_ID_NONE;
...
static bool isCodecSupportedByStagefright(enum AVCodecID codec_id)
{
    ...
    case AV_CODEC_ID_PCM_U8:			// wav

	case AV_CODEC_ID_PCM_S16LE:		// wav
	case AV_CODEC_ID_PCM_U16LE:		// wav

    case AV_CODEC_ID_PCM_S24LE:		// wav
	case AV_CODEC_ID_PCM_U24LE:		// wav

    case AV_CODEC_ID_PCM_S32LE:		// wav
	case AV_CODEC_ID_PCM_U32LE:		// wav
        supported = true;
        break;
    ...
    ALOGV("### %ssupported codec(%s) by official Stagefright ###",
            (supported ? "" : "un"),
...
static const char *findMatchingContainer(const char *name)
{
	...
	ALOGV("### %s() / name=%s / container=%s ###", __FUNCTION__, name, (container == NULL) ? "NULL" : container);
...
static const char *SniffFFMPEGCommon(const char *url, float *confidence, bool isStreaming)
{
    ...
    ALOGD("### url=%s / format_name=%s / format_long_name=%s ###",
    ...
    if (container) {
		...
		// 2016.12.22 iriver-eddy
		ALOGV("### %s() / confidence = %.2f ###", __FUNCTION__, *confidence);
    }
        ...
        float *confidence, sp<AMessage> meta)
{
    ...
    ALOGV("\t### %s() / android-source:%p ###", __FUNCTION__, source.get());
         ...
         float *confidence, sp<AMessage> meta)
{
    ...
    ALOGD("### %s() / %s ###", __FUNCTION__, uri.string());
...
bool SniffFFMPEG(
    ...
    ALOGD("### %s() / initial confidence=%.2f / mime=%s ###", __FUNCTION__, *confidence, mimeType == NULL ? "unknown" : *mimeType);
    ...
    if (mimeType != NULL && confidence != NULL) {
        if (*mimeType == "application/ogg") {
			ALOGV("\t### %s() / mimeType == application/ogg ###", __FUNCTION__);
            return false;
        }
        if (*confidence > 0.8f) {
			ALOGV("\t### %s() / confidence is over 0.8f ###", __FUNCTION__);
            return false;
        }
    }
    ...
    *meta = new AMessage;
    ...
    if (!container) {
        ALOGW("### %s() / sniff through BetterSniffFFMPEG failed / try LegacySniffFFMPEG ###", __FUNCTION__);
        container = LegacySniffFFMPEG(source, &newConfidence, *meta);
        if (container) {
            ALOGV("### %s() / sniff through LegacySniffFFMPEG success ###", __FUNCTION__);
        }
    } else {
        ALOGV("### %s() / sniff through BetterSniffFFMPEG success ###", __FUNCTION__);
    }

    if (container == NULL) {
        ALOGW("### %s() / SniffFFMPEG failed to sniff this source ###", __FUNCTION__);
        (*meta)->clear();
        *meta = NULL;
        return false;
    }

    ALOGD("### %s() / FFmpeg detected media content as '%s' with confidence %.2f ###",
            __FUNCTION__, container, newConfidence);
...
// 2016.12.05 iriver-eddy #ADD_CODEC, #WMV_PLAYBACK
		...
		ALOGV("\t### %s() / new FFmpegExtractor() ###", __FUNCTION__);
