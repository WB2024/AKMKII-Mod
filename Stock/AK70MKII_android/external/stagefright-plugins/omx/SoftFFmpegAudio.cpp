...
SoftFFmpegAudio::SoftFFmpegAudio(
    ...
    ALOGD("### %s() / component=%s / mCodingType=%d ###",
            __FUNCTION__, name, mCodingType);
...
void SoftFFmpegAudio::initPorts() {
    ...
    if (mCodingType == (OMX_AUDIO_CODINGTYPE)OMX_AUDIO_CodingAPE) {
		// 2016.11.09 iriver-eddy #HIFI, #APE_PLAYBACK
//        def.nBufferSize = 1000000; // ape!
        def.nBufferSize = (4 * 1024 * 1024); // ape!
    } else if (mCodingType == (OMX_AUDIO_CODINGTYPE)OMX_AUDIO_CodingDTS) {
        def.nBufferSize = (4 * 1024 * 1024); // dts!
    } else {
        // max aggregated buffer size from nuplayer
		// 2016.11.09 iriver-eddy #HIFI
        ...
        case OMX_IndexParamAudioPcm:
        {
            ...
            if (isConfigured()) {
                ...
                if (packed == AV_SAMPLE_FMT_U8) {
					...
					// 2016.11.23 iriver-eddy #WMA_PLAYBACK
//                    profile->nBitPerSample = 24;
            ...
            ALOGV("### %s() / get pcm params / nChannels=%u / nSamplingRate=%u / nBitsPerSample=%u / mAudioTgtFmt=%s(%d) ###",
                   __FUNCTION__, profile->nChannels, profile->nSamplingRate, profile->nBitPerSample, (mAudioTgtFmt != AV_SAMPLE_FMT_NONE) ? av_get_sample_fmt_name(mAudioTgtFmt) : "NONE", mAudioTgtFmt);
		...
		case OMX_IndexParamAudioWma:
        {
            ...
            ALOGV("### %s() / get WMA params / nChannels=%u / nSamplingRate=%u / nBitsRate=%u ###",
                   __FUNCTION__, profile->nChannels, profile->nSamplingRate, profile->nBitRate);
		...
		case OMX_IndexParamAudioApe:
        {
            ...
            ALOGV("### %s() / get APE params / nChannels=%u / nSamplingRate=%u / nBitsPerSample=%u ###",
                   __FUNCTION__, profile->nChannels, profile->nSamplingRate, profile->nBitsPerSample);
...
//    ALOGV("### internalSetParameter index:0x%x ###", index);
    ...
    switch ((int)index) {
        ...
        case OMX_IndexParamAudioPcm:
        {
            ...
            ALOGV("### %s() / set OMX_IndexParamAudioPcm / nChannels=%u / "
                    "nSampleRate=%u / nBitsPerSample=%u / mAudioTgtFmt=%s ###",
                __FUNCTION__, profile->nChannels, profile->nSamplingRate,
                profile->nBitPerSample, av_get_sample_fmt_name(mAudioTgtFmt));
       ...
       case OMX_IndexParamAudioWma:
        {
            ...
            ALOGV("### %s() / set OMX_IndexParamAudioWma / nChannels=%u / "
		...
		case OMX_IndexParamAudioApe:
        {
            ...
            ALOGV("### %s() / set OMX_IndexParamAudioApe / nChannels=%u / "
...
void SoftFFmpegAudio::adjustAudioParams() {
	...
	// 2016.11.08 iriver-eddy [FIXME] #HIFI
//    mAudioTgtFreq = FFMIN(192000, FFMAX(8000, mCtx->sample_rate));
    mAudioTgtFreq = FFMIN(384000, FFMAX(8000, mCtx->sample_rate));
    ...
    ALOGD("### %s() / [channels=%d freq=%d fmt=%s] ###",
            __FUNCTION__, mCtx->channels, mCtx->sample_rate, av_get_sample_fmt_name(mAudioTgtFmt));
}
...
int32_t SoftFFmpegAudio::handleExtradata() {
    ...
    ALOGV("got extradata, ignore: %d, size: %u",
...
int32_t SoftFFmpegAudio::openDecoder() {
    ...
    if (!mExtradataReady && !mIgnoreExtradata) {
        ...
        ALOGV("extradata is ready, size: %d", mCtx->extradata_size);
    ...
    while (!outQueue.empty()) {
        if (mResampledDataSize == 0) {
            ...
            if (err < ERR_OK) {
                ...
                return;
            } else if (err == ERR_FLUSHED || err == ERR_NO_FRM) {   				// 2016.12.01 iriver-eddy
                drainEOSOutputBuffer();
                return;
            } else {
                CHECK_EQ(err, ERR_OK);
