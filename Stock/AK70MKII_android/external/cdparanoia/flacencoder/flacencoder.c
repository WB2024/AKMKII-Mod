...
typedef struct {
    ...
    FLAC__byte KeyAlmEncode[METADATA_MAX_LENGTH];
    FLAC__byte KeyAlmOriginalSampleRate[METADATA_MAX_LENGTH];
...
FLAC_ENCODER_STATUS wav_encodeTo_flac_finish_meta() {
    ...
    if(gMetaData->metaNum > 0) {
        if((metadata[META_VORBIS_TAGS] = FLAC__metadata_object_new(FLAC__METADATA_TYPE_VORBIS_COMMENT)) != NULL) {
			...
			// 2018.07.18 iriver-eddy #MQA_DECODE
#if defined(_SUPPORT_MQA_DECODE_)
            if ((strlen((char *)gMetaData->KeyAlmEncode) > 0)) {
                if (debug_log) ALOGI("#### gMetaData->KeyAlmEncode : %s, %d", 
                    gMetaData->KeyAlmEncode, strlen((char *)gMetaData->KeyAlmEncode));
                retResult = FLAC__metadata_object_vorbiscomment_entry_from_name_value_pair(
                    &entry, "ENCODER", (const char *)gMetaData->KeyAlmEncode);
                retResult &= FLAC__metadata_object_vorbiscomment_append_comment(
                    metadata[META_VORBIS_TAGS], entry, false);
                if (retResult != 1) {
                    ALOGE("wav_encodeTo_flac_finish_meta ### ERROR! Vorbis KeyAlmEncode Meta"); 
                    return FLAC_ENCODER_STATUS_ERROR_META;
                }
            }
            if ((strlen((char *)gMetaData->KeyAlmOriginalSampleRate) > 0)) {
                if (debug_log) ALOGI("#### gMetaData->KeyAlmOriginalSampleRate : %s, %d", 
                    gMetaData->KeyAlmOriginalSampleRate, strlen((char *)gMetaData->KeyAlmOriginalSampleRate));
                retResult = FLAC__metadata_object_vorbiscomment_entry_from_name_value_pair(
                    &entry, "ORIGINALSAMPLERATE", (const char *)gMetaData->KeyAlmOriginalSampleRate);
                retResult &= FLAC__metadata_object_vorbiscomment_append_comment(
                    metadata[META_VORBIS_TAGS], entry, false);
                if (retResult != 1) {
                    ALOGE("wav_encodeTo_flac_finish_meta ### ERROR! Vorbis KeyAlmOriginalSampleRate Meta"); 
                    return FLAC_ENCODER_STATUS_ERROR_META;
                }
            }
#endif // #if defined(_SUPPORT_MQA_DECODE_)
...
FLAC_ENCODER_STATUS wav_encodeTo_flac_add_meta(FLAC_METADATA_INFO info, const char *infoData) {
    ...
    switch(info) {
		...
		// 2018.07.18 iriver-eddy #MQA_DECODE
#if defined(_SUPPORT_MQA_DECODE_)
		case FLAC_METADATA_ENCODER:
            memset(gMetaData->KeyAlmEncode, 0, METADATA_MAX_LENGTH);
            memcpy(gMetaData->KeyAlmEncode, infoData, info_length);
		break;
		case FLAC_METADATA_ORIGINAL_SAMPLERATE:
            memset(gMetaData->KeyAlmOriginalSampleRate, 0, METADATA_MAX_LENGTH);
            memcpy(gMetaData->KeyAlmOriginalSampleRate, infoData, info_length);
		break;
#endif // #if defined(_SUPPORT_MQA_DECODE_)
		// /////////////////////////////////////////////////////////////////////////////////////////////////////////
        default:
