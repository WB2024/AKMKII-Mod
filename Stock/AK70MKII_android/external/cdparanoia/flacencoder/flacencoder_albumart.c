...
#define LOG_NDEBUG 0
#define TAG "FLACEncoderAlbumArt"
...
PictureFile picturefiles[] = {
    ...
    { "8.png", "image/png" , "", 32, 32, 32, 0, FLAC__STREAM_METADATA_PICTURE_TYPE_UNDEFINED }
};
...
static FLAC__bool debug_ = true;
static const char preFixPath[21] = "/storage/emulated/0";
...
//Test Code
////////////////////////////////////////////
int testImageInsertFlac(const char *prefix) {
    FLAC__StreamMetadata *obj;
    const char *error;
    size_t i;

    ALOGI("\n testImageInsertFlac : picture\n\n");
#if 0
    /* invalid spec: no filename */
    ALOGE("testing grabbag__picture_parse_specification(\"\")... ");
    if(0 != (obj = grabbag__picture_parse_specification("", &error)))
        return failed_("expected error, got object");
    ALOGE("OK (failed as expected, error: %s)\n", error);

    /* invalid spec: no filename */
    ALOGE("testing grabbag__picture_parse_specification(\"||||\")... ");
    if(0 != (obj = grabbag__picture_parse_specification("||||", &error)))
        return failed_("expected error, got object");
    ALOGE("OK (failed as expected: %s)\n", error);

    /* invalid spec: no filename */
    ALOGE("testing grabbag__picture_parse_specification(\"|image/gif|||\")... ");
    if(0 != (obj = grabbag__picture_parse_specification("|image/gif|||", &error)))
        return failed_("expected error, got object");
    ALOGE("OK (failed as expected: %s)\n", error);

    /* invalid spec: bad resolution */
    ALOGE("testing grabbag__picture_parse_specification(\"|image/gif|desc|320|0.gif\")... ");
    if(0 != (obj = grabbag__picture_parse_specification("|image/gif|desc|320|/storage/emulated/0/0.gif", &error)))
        return failed_("expected error, got object");
    ALOGE("OK (failed as expected: %s)\n", error);

    /* invalid spec: bad resolution */
    ALOGE("testing grabbag__picture_parse_specification(\"|image/gif|desc|320x240|0.gif\")... ");
    if(0 != (obj = grabbag__picture_parse_specification("|image/gif|desc|320x240|/storage/emulated/0/0.gif", &error)))
        return failed_("expected error, got object");
    ALOGE("OK (failed as expected: %s)\n", error);

    /* invalid spec: no filename */
    ALOGE("testing grabbag__picture_parse_specification(\"|image/gif|desc|320x240x9|\")... ");
    if(0 != (obj = grabbag__picture_parse_specification("|image/gif|desc|320x240x9|", &error)))
        return failed_("expected error, got object");
    ALOGE("OK (failed as expected: %s)\n", error);

    /* invalid spec: #colors exceeds color depth */
    ALOGE("testing grabbag__picture_parse_specification(\"|image/gif|desc|320x240x9|0.gif\")... ");
    if(0 != (obj = grabbag__picture_parse_specification("|image/gif|desc|320x240x9|/storage/emulated/0/0.gif", &error)))
        return failed_("expected error, got object");
    ALOGE("OK (failed as expected: %s)\n", error);

    /* invalid spec: standard icon has to be 32x32 PNG */
    ALOGE("testing grabbag__picture_parse_specification(\"1|-->|desc|32x24x9|0.gif\")... ");
    if(0 != (obj = grabbag__picture_parse_specification("1|-->|desc|32x24x9|/storage/emulated/0/0.gif", &error)))
        return failed_("expected error, got object");
    ALOGE("OK (failed as expected: %s)\n", error);

    /* invalid spec: need resolution for linked URL */
    ALOGE("testing grabbag__picture_parse_specification(\"|-->|desc||http://blah.blah.blah/z.gif\")... ");
    if(0 != (obj = grabbag__picture_parse_specification("|-->|desc||http://blah.blah.blah/z.gif", &error)))
        return failed_("expected error, got object");
    ALOGE("OK (failed as expected: %s)\n", error);

    ALOGE("testing grabbag__picture_parse_specification(\"|-->|desc|320x240x9|http://blah.blah.blah/z.gif\")... ");
...
static FLAC__bool test_one_picture(const char *prefix, const PictureFile *pf, const char *res, FLAC__bool fn_only)
{
    ...
    if(debug_log) ALOGI("testing grabbag__picture_parse_specification(\"%s\")... ", s);
...
static FLAC__bool local__parse_type_(const char *s, size_t len, FLAC__StreamMetadata_Picture *picture)
{
    ...
    if(i == len)
        picture->type = (FLAC__StreamMetadata_Picture_Type) val;
    else
        return false;
...
FLAC__off_t grabbag__file_get_filesize(const char *srcpath)
{
    ...
    if(debug_log) ALOGI("testing grabbag__file_get_filesize srcpath : %s ", srcpath);
        ...
        if(state != 4)
            *error_message = error_messages[1];
            ...
            if(0 == strcmp(obj->data.picture.mime_type, "-->")) { /* magic MIME type means URL */
                ...
                if(debug_log) ALOGI("grabbag__picture_parse_specification size : %lu ", size);
                    ...
                    if(0 == buffer)
                        *error_message = error_messages[0];
                        ...
                        if(0 == f) {
                            *error_message = error_messages[5];
                            free(buffer);
                            ALOGE("#### grabbag__picture_parse_specification #### Error!");
                        }
                        else {
                            ...
                            if(0 == *error_message) {
                                if(!FLAC__metadata_object_picture_set_data(obj, buffer, size, /*copy=*/false)) {
                                    *error_message = error_messages[6];
                                    ALOGE("#### grabbag__picture_parse_specification #### data set Error!");
                                }
                                /* try to extract MIME type if user left it blank */
                                    ...
                                    *error_message = error_messages[8];
                                    ALOGE("#### grabbag__picture_parse_specification #### type Error!");
                                }
                                /* try to extract resolution/color info if user left it blank */
                                    ...
                                    *error_message = error_messages[4];
                                    ALOGE("#### grabbag__picture_parse_specification #### size Error!");                           }
                                }
                            else {
                                ALOGE("#### grabbag__picture_parse_specification #### already Error!");
                                free(buffer);
                            }
                        }
                    }
    ...
    if(*error_message == 0) {
        if(
            ...
            *error_message = error_messages[9];
            ALOGE("#### grabbag__picture_parse_specification #### PNG Error!");
       }
    }
...
FLAC__StreamMetadata *grabbag__picture_parse_buffer_specification(const char *buffer, const unsigned long length, const char **error_message)
{
    FLAC__StreamMetadata *obj;
    int state = 0;
    static const char *error_messages[] = {
    ...
    *error_message = 0;
    ...
    if(0 == (obj = FLAC__metadata_object_new(FLAC__METADATA_TYPE_PICTURE))) {
     ...
     /* simple format, filename only, everything else guessed */
    if(!local__parse_type_("", 0, &obj->data.picture)) /* use default picture type */
        *error_message = error_messages[7];
    /* leave MIME type to be filled in later */
    /* leave description empty */
    ...
    if(*error_message == 0) {
        if(state != 4)
            *error_message = error_messages[1];
            ...
            /* regular picture file */
            const FLAC__off_t size = (FLAC__off_t)length;

            if(debug_log) ALOGI("grabbag__picture_parse_buffer_specification size : %lu ", size);
            
            if(size < 0)
                *error_message = error_messages[5];
            else {
                FLAC__byte *bufferIn = (FLAC__byte *)safe_malloc_(size + 1);
                if(0 == buffer)
                    *error_message = error_messages[0];
                else {
                    if(0 == *error_message) {
                        memset(bufferIn, 0, size);
                        memcpy(bufferIn, buffer, size);
                        if(!FLAC__metadata_object_picture_set_data(obj, bufferIn, size, /*copy=*/false)) {
                            *error_message = error_messages[6];
                            ALOGE("#### grabbag__picture_parse_buffer_specification #### data set Error!");
                        }
                        /* try to extract MIME type if user left it blank */
                        else if(*obj->data.picture.mime_type == '\0' && !local__extract_mime_type_(obj)) {
                            *error_message = error_messages[8];
                            ALOGE("#### grabbag__picture_parse_buffer_specification #### type Error!");
                        }
                        /* try to extract resolution/color info if user left it blank */
                        else if((obj->data.picture.width == 0 || obj->data.picture.height == 0 || obj->data.picture.depth == 0) && !local__extract_resolution_color_info_(&obj->data.picture)) {
                            *error_message = error_messages[4];
                            ALOGE("#### grabbag__picture_parse_buffer_specification #### size Error!");                           
                        }
                    } else {
                        ALOGE("#### grabbag__picture_parse_buffer_specification #### already Error!");
                        free(bufferIn);
                    }
                }
            }
    ...
    if(*error_message == 0) {
        if(
            ...
            *error_message = error_messages[9];
            ALOGE("#### grabbag__picture_parse_buffer_specification #### PNG Error!");
       }
    }
