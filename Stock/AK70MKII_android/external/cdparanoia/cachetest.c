...
static int time_drive(cdrom_drive *d, FILE *progress, FILE *log, int lba,
	 ...
	 optimization when sleeping. */
	{
		...
		if (initial_seek) {
			printC(
					"%4dms seek, %.2fms/sec read [%.1fx]", latency, mean, 1000./75./mean);
...
static float retime_drive(cdrom_drive *d, FILE *progress, FILE *log, int lba,
...
int analyze_cache(cdrom_drive *d, FILE *progress, FILE *log, int speed) {
	 ...
	 eyes.  Take readings ten minutes apart (45000 sectors) and at end of disk. */
	{
		...
		while (offset >= firstsector) {
			...
			if ((ret = cdda_read(d, NULL, offset + current + 1, 1)) < 0) {
				...
				offset = (offset - firstsector + 44999) / 45000 * 45000
						+ firstsector;
				offset -= 45000;
				continue;
			}
	 ...
	 bisection */
	{
		...
		while (current <= hi && under) {
			...
			printC("\r");
			reportC(
					"\tFast search for approximate cache size... %d sectors            ", current-1);
			logC("\n");
				...
				for (j = 0;; j++) {
					...
					if (i >= 15) {
						...
						if (i == 15) {
							printC("\r");
							reportC(
									"\tSlow verify for approximate cache size... %d sectors", current-1);
							logC("\n");
						...
						while (sofar < current) {
							ret1 = cdda_read_timed(d, NULL, offset + sofar,
									current - sofar, &x);
						...
						ret1 = cdda_read_timed(d, NULL, offset + current - 1, 1,
								&x);
						logC(
								"\t\t>>> fast_read=%d:%d:%d ", offset+current-1, ret1, x);
...

	{
		...
		while (gran > 1 || under) {
			...
			logC("\n");
			reportC(
					"\tTesting background readahead past read cursor... %d", readahead);
			...
			for (i = 0; i < it; i++) {
				...
				ret = cdda_read_timed(d, NULL,
						offset + cachesize + readahead - 1, 1, &x);
				if (ret <= 0)
					break;
				...
				if (x < MIN_SEEK_MS) {
					...
					mspersector = retime_drive(d, progress, log, offset,
							readahead, mspersector);
				}
			}
	...
	while (1) {
		...
		{
			float newms = retime_drive(d, progress, log, offset, readahead,
					mspersector);
		...
		if (readsize < 7) {
...

			{
				...
				while (elapsed < 5000) {
					...
					printC(
							"\b%c", (spinner==0?'o':(spinner==1?'O':(spinner==2?'o':'.'))));
				}
				printC("\r");
				logC("\n");
				...
				reportC("\tTesting that backseek flushes cache...");
				{
					...
					while (elapsed < 5000 && total < 25) { /* don't kill the drive */
						...
						while (1) {
							...
							if (ret > 0)
								ret = time_drive(d, NULL, log, offset + 1,
										readsize, 1);
							if (ret >= 0)
								ret = time_drive(d, NULL, log, offset, readsize,
										1);
