...
extern TAG_GENERATOR_STATUS wav_tag_generator_gen_init(
        const int fileFd,
        const unsigned char *inWavfilename, 
        const void *file, 
        const unsigned long long offset,
        const WAV_SOURCE_FROM mode);
extern TAG_GENERATOR_STATUS wav_tag_generator_gen_add_meta(
        const WAV_METADATA_INFO info, 
        const unsigned char *field_value);
extern TAG_GENERATOR_STATUS wav_tag_generator_gen_meta_write();
extern TAG_GENERATOR_STATUS wav_tag_generator_gen_add_albumart(
        const WAV_METADATA_INFO info, 
        const unsigned char *buffer, 
        const unsigned long length);
extern TAG_GENERATOR_STATUS wav_tag_generator_gen_finish();

#ifdef _WAV_GENERATOR_TEST_SAMPLE_CODE_
extern void wav_tag_tagGenerator_TEST();
#endif
