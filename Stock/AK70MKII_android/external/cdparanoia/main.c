...
#include <sys/stat.h>
#include <utils/Log.h>
#include "flacencoder/flacencoder.h"
    ...
    for (i = 1; i <= d->tracks; i++)
        if (cdda_track_audiop(d, i) > 0) {
            ...
            report(
                    ...
                    i, off, (int) (off / (60 * 75)), (int) ((off / 75) % 60),
                    (int) (off % 75), sec, (int) (sec / (60 * 75)),
                    (int) ((sec / 75) % 60), (int) (sec % 75),
...
static char *callback_strings[16] = { "wrote", "finished", "read", "verify",
        "jitter", "correction", "scratch", "scratch repair", "skip", "drift",
        "backoff", "overlap", "dropped", "duped", "transport error",
        "cache error" };

static int skipped_flag = 0;
static int abort_on_skip = 0;
static int exit_ripping = 0;
FILE *logfile = NULL;
static void callback(long inpos, int function) {
    ...
    if (callscript)
        fprintf(stderr, "##: %d [%s] @ %ld\n", function,
                (function >= -2 && function <= 13 ?
                        callback_strings[function + 2] : ""), inpos);
    else {
    ...
    if (!quiet) {
        ...
        if (printit == 1) { /* else don't bother; it's probably being 
         redirected */
            position = ((float) (sector - callbegin) / (callend - callbegin))
                    * graph;
            ...
            case 8: /* skip */
                smilie = " ;-(";
                skipped_flag = 1;
                report("### [Ripper] callback slevel is 8 / set skip flag ###\n");
                break;
            ...
            if (lasttime != test || function == -1 || slast != slevel) {
                ...
                if (abort_on_skip && skipped_flag && function != -1) {
                    ...
                    if (v_sector == 0)
                        sprintf(buffer,
                                ...
                                dispcache, overlap / CD_FRAMEWORDS, smilie,
                                heartbeat);

                    else
                        sprintf(buffer,
                                "\r (== PROGRESS == [%s| %06ld %02d ] ==%s %c ==)   ",
                                dispcache, v_sector, overlap / CD_FRAMEWORDS,
                                smilie, heartbeat);
                ...
                fprintf(stderr, "%s", (const char*) buffer); // 2014-3-21 jhlim

                if (logfile != NULL && function == -1) {
                    fprintf(logfile, "%s", (char*) buffer + 1);
                    fprintf(logfile, "\n\n");
                    fflush(logfile);
                }
            }
...
struct option options[] = { { "stderr-progress", no_argument, NULL, 'e' }, {
        ...
        "force-read-speed", required_argument, NULL, 'S' }, { "sample-offset",
        required_argument, NULL, 'O' }, { "toc-offset", required_argument, NULL,
        't' }, { "toc-bias", no_argument, NULL, 'T' }, { "output-raw",
        no_argument, NULL, 'p' }, { "output-raw-little-endian", no_argument,
        NULL, 'r' }, { "output-raw-big-endian", no_argument, NULL, 'R' }, {
        "output-wav", no_argument, NULL, 'w' }, { "output-aiff", no_argument,
        NULL, 'f' }, { "output-aifc", no_argument, NULL, 'a' }, { "batch",
        no_argument, NULL, 'B' }, { "verbose", no_argument, NULL, 'v' }, {
        "quiet", no_argument, NULL, 'q' },
        { "version", no_argument, NULL, 'V' },
        { "query", no_argument, NULL, 'Q' }, { "help", no_argument, NULL, 'h' },
        { "analyze-drive", no_argument, NULL, 'A' }, { "disable-paranoia",
                no_argument, NULL, 'Z' }, { "disable-extra-paranoia",
                no_argument, NULL, 'Y' }, { "abort-on-skip", no_argument, NULL,
                'X' }, { "disable-fragmentation", no_argument, NULL, 'F' }, {
                "output-info", required_argument, NULL, 'i' }, { "never-skip",
                optional_argument, NULL, 'z' }, { "log-summary",
                optional_argument, NULL, 'l' }, { "log-debug",
...
/*
 ...
 ///////////////////////////////////////////////////////////////////////
 * -l or -L    ===>   create log file
 * -d "/dev/block/sr0"   ===>   device select
 * -S 4   ===>   4x speed read
 * -A   ===>   Analyze disc information (check model, verify, read or seek speed, caches...)    
 * -Q   ===>   autosense and qurey disc (TOC information)
 ///////////////////////////////////////////////////////////////////////
 */
...
int main(int argc, char *argv[]) {
    ...
    if (force_cooked_device) {
        ...
        d = cdda_identify_scsi(force_generic_device, force_cdrom_device,
                verbose, NULL);
        ...
        /* does the /dev/block/sr0 link exist? */
        struct stat s;
        if (lstat("/dev/block/sr0", &s)) {
            /* no link.  Search anyway */
            d = cdda_find_a_cdrom(verbose, NULL);
        } else {
            d = cdda_identify("/dev/block/sr0", verbose, NULL);
            if (d == NULL && !verbose) {
                verbose = 1;
                report(
                        "\n/dev/block/sr0 exists but isn't accessible.  By default,\n"
                                "cdparanoia stops searching for an accessible drive here.\n"
                                "Consider using -sv to force a more complete autosense\n"
                                "of the machine.\n\nMore information about /dev/block/sr0:");

                d = cdda_identify("/dev/block/sr0", CDDA_MESSAGE_PRINTIT, NULL);
                report("\n");
                exit(1);
            } else
                report(" ");
...

    {
            ...
            if (first_sector == -1) {
                ...
                if (last_sector == -1) {
                    if (span2) { /* There was a hyphen */
                        ...
                        last_sector = cdda_track_lastsector(d,
                                cdda_sector_gettrack(d, first_sector));
                    }
                }
            }
        } else {
...

        {
            ...
            report("Ripping from sector %7ld (track %2d [%d:%02d.%02d])\n"
                    ...
                    first_sector, track1, (int) (off1 / (60 * 75)),
                    (int) ((off1 / 75) % 60), (int) (off1 % 75), last_sector,
                    track2, (int) (off2 / (60 * 75)), (int) ((off2 / 75) % 60),
                    (int) (off2 % 75));
            ...
            while (cursor <= last_sector) {
                ...
                if (optind + 1 < argc) {
                    if (!strcmp(argv[optind + 1], "-")) {
                        ...
                        //20140410 WAV_TO_FLAC_ENCODING
                        if(FLAC_CHECK_FLAG != 1) {
                            out = open(outfile_name, O_RDWR | O_CREAT | O_TRUNC,
                                    0666);
                            if (out == -1) {
                    ...
                    if(FLAC_CHECK_FLAG != 1) {
                        ...
                        if (out == -1) {
                            report("Cannot open default output file %s: %s",
                                    outfile_name, strerror(errno));
                            cdda_close(d);
                            d = NULL;
                            exit(1);
                ...
                case 1: /* wav */
                    WriteWav(out,
                            (batch_last - batch_first + 1) * CD_FRAMESIZE_RAW);
                    break;
                case 2: /* aifc */
                    WriteAifc(out,
                            (batch_last - batch_first + 1) * CD_FRAMESIZE_RAW);
                ...
                case 4: /* flac */
                    wav_encodeTo_flac_Open();
                    //FLAC Header
                    wav_encodeTo_flac_Create_Header((batch_last - batch_first + 1) * CD_FRAMESIZE_RAW,
                        2, 44100, 16, 0, 1);                    
                    //MetaData Vorbis
                    wav_encodeTo_flac_init_meta();
                    wav_encodeTo_flac_add_meta(FLAC_METADATA_ALBUM, "HOT PINK BOY");
                    wav_encodeTo_flac_add_meta(FLAC_METADATA_ARTIST, "HOT RED LADY");
                    wav_encodeTo_flac_add_meta(FLAC_METADATA_TITLE, "CDRipping");
                    wav_encodeTo_flac_add_meta(FLAC_METADATA_ALBUMARTIST, "IRIVER 01");
                    wav_encodeTo_flac_add_meta(FLAC_METADATA_COMPOSER, "IRIVER 02");
                    wav_encodeTo_flac_add_meta(FLAC_METADATA_TRACKNUM, "0001");    
                    wav_encodeTo_flac_add_meta(FLAC_METADATA_GENRE, "ROCK N ROLL");
                    wav_encodeTo_flac_add_meta(FLAC_METADATA_YEAR, "2014");
                    //AlbumArt Example
#if 1 //File Path only
                    wav_encodeTo_flac_add_AlbumArt(FLAC_METADATA_ALBUMART_PATH, NULL/*"/storage/emulated/0/0.gif"*/, NULL, 0, 3);
#else //Data Buffer & Length
                    struct stat srcstat;  
                    unsigned long lLength = 0;
                    if(0 == stat("/storage/emulated/0/0.gif", &srcstat)) {
                        lLength = srcstat.st_size;    
                        unsigned char *lBuffer = (unsigned char *)malloc(lLength + 1);
                        FILE *f = fopen("/storage/emulated/0/0.gif", "rb");
                        if(0 == f) {
                            if(lBuffer != NULL) free(lBuffer);
                        } else {                    
                            if(fread(lBuffer, 1, lLength, f) == (size_t)lLength) {
                                //////////////////////////////
                                wav_encodeTo_flac_add_AlbumArt(FLAC_METADATA_ALBUMART_DATA, NULL, (const unsigned char *)lBuffer, lLength, 3);
                                //////////////////////////////
                            }
                            fclose(f);
                        }
...
#endif
                    wav_encodeTo_flac_finish_meta();
                    //Raw Data
                    wav_encodeTo_flac_EncodeBufferInit(outfile_name);
                    break;
                ...
                if (offset_buffer_used) {
                    ...
                    cursor++;
                    //20140410 WAV_TO_FLAC_ENCODING 
                    if(FLAC_CHECK_FLAG == 1) { /* flac */
                        wav_encodeTo_flac_EncodeBuffer(((unsigned char *) offset_buffer) + offset_buffer_used,
                ...
                while (cursor <= batch_last) {
                    /* read a sector */
                    int16_t *readbuf = paranoia_read_limited(p, &exit_ripping, callback,
                            max_retries);
                    ...
                    if (skipped_flag && abort_on_skip) {
                    		report("### skipped_flag && abort_on_skip 1 ###\n");
                        cursor = batch_last + 1;
                        break;
                    }
                    ...
                    //20140410 WAV_TO_FLAC_ENCODING 
                    if(FLAC_CHECK_FLAG == 1) { /* flac */
                        wav_encodeTo_flac_EncodeBuffer(((unsigned char *) readbuf) + offset_skip,
                    ...
                    if (sample_offset && cursor > batch_last) {
                        ...
                        readbuf = paranoia_read_limited(p, &exit_ripping, callback,
                                max_retries);
                        err = cdda_errors(d);
                        mes = cdda_messages(d);
                        ...
                        if (skipped_flag && abort_on_skip) {
                        		report("### skipped_flag && abort_on_skip 2 ###\n");
                            break;
                         }
                        skipped_flag = 0;
                        ...
                        //20140410 WAV_TO_FLAC_ENCODING 
                        if(FLAC_CHECK_FLAG == 1) { /* flac */
                            wav_encodeTo_flac_EncodeBuffer((unsigned char *) offset_buffer,
                                offset_buffer_used);
                        } else {
