...
static inline long i_paranoia_overlap(int16_t *buffA, int16_t *buffB,
		long offsetA, long offsetB, long sizeA, long sizeB, long *ret_begin,
		long *ret_end) {
...
static inline long i_paranoia_overlap2(int16_t *buffA, int16_t *buffB,
		unsigned char *flagsA, unsigned char *flagsB, long offsetA,
		long offsetB, long sizeA, long sizeB, long *ret_begin, long *ret_end) {
	long beginA = offsetA, endA = offsetA;
	long beginB = offsetB, endB = offsetB;
 ...
 */
static inline long do_const_sync(c_block *A, sort_info *B, unsigned char *flagB,
		long posA, long posB, long *begin, long *end, long *offset) {
	 ...
	 */
	if (flagB == NULL)
		ret = i_paranoia_overlap(cv(A), iv(B), posA, posB, cs(A), is(B), begin,
				end);
...
static inline long try_sort_sync(cdrom_paranoia *p, sort_info *A,
		unsigned char *Aflags, c_block *B, long post, long *begin, long *end,
		...
		/* always try absolute offset zero first! */
		{
			...
			if (zeropos >= 0 && zeropos < is(A)) {
				...
				if (cv(B)[post - cb(B)] == iv(A)[zeropos]) {
					 ...
					 */
					if (do_const_sync(B, A, Aflags, post - cb(B), zeropos,
							begin, end, offset)) {
	...
	if (!rv(root)) {
		...
		if (i_iterate_stage2(p, v, &r, callback)) {
			...
			while ((begin + offset > 0 && begin > 0)) {
				 ...
				 */
				i_paranoia_overlap(rv(root), cv(l), begin, beginL, rs(root),
						cs(l), &begin, &end);
			...
			while (end + offset < temp && end < rs(root)) {
				 ...
				 *             the fragment and root agree and are in sync
				 */
				i_analyze_rift_f(rv(root), cv(l), rs(root), cs(l), end, endL,
						&matchA, &matchB, &matchC);
				...
				if (matchA) {
					 ...
					 */
					analyze_rift_silence_f(rv(root), cv(l), rs(root), cs(l),
							end, endL, &matchA, &matchB);
					if (matchA) {
			 ...
			 */
			{
				...
				if (sizeB - offset > sizeA || v->lastsector) {
					 ...
					 * indicates jitter.
					 */
					offset_add_value(p, &p->stage2,
							offset + vecbegin - rb(root), callback);
				}
			}
			if (l)
				i_cblock_destructor(l);
		...
		if (count) {
			...
			for (count = 0; count < active; count++) {
				...
				if (first->one) {
					...
					if (rv(root) == NULL) {
						if (i_init_root(&(p->root), first, beginword,
								callback)) {
		...
		while (c) {
			...
			if (cbegin <= post && cend > post) {
				...
				if (c->flags[post - cbegin] & FLAGS_VERIFIED) {
					/* verified area! */
					while (vend < cend
							&& (c->flags[vend - cbegin] & FLAGS_VERIFIED))
						vend++;
					if (!vflag || vend > vflag) {
						graft = c;
						gend = vend;
...
static void cdrom_cache_handler(cdrom_paranoia *p, int lba,
		void (*callback)(long, int)) {
	int seekpos;
	int ms;
	...
	if (lba < p->cdcache_begin) {
		...
		seekpos = (
				lba == 0 || lba - 1 < cdda_disc_firstsector(p->d) ?
						lba : lba - 1); /* keep reads linear when possible */
	} else {
...
c_block *i_read_c_block(cdrom_paranoia *p, int *exit_ripping, long beginword, long endword,
		void (*callback)(long, int)) {
	...
	while (sofar < totaltoread) {
		...
		if (secread > 0) {
			...
			// 2014.11.18 iriver-eddy
//			ALOGD("### Paranoia / cdda_read() ###");

			if ((thisread = cdda_read(p->d, buffer + sofar * CD_FRAMEWORDS,
					adjread, secread)) < secread) {

				// 2014.11.17 iriver-eddy
				ALOGW("### Failed Paranoia / cdda_read() / exit_ripping = %d ###", *exit_ripping);

				// 2014.07.17 iriver-eddy
				if (*exit_ripping == 1) {
					/* the one error we bail on immediately */
					if (new)free_c_block(new);
					if (buffer)
				...
				if (flags)
					memset(flags + (sofar + thisread) * CD_FRAMEWORDS,
							FLAGS_UNREAD, CD_FRAMEWORDS * (secread - thisread));
			}
			if (thisread != 0)
				anyflag = 1;
			...
			if (callback)
				(*callback)((adjread + secread - 1) * CD_FRAMEWORDS,
						PARANOIA_CB_READ);
...
int16_t *paranoia_read(cdrom_paranoia *p, int *exit_ripping, void (*callback)(long, int)) {
	return paranoia_read_limited(p, exit_ripping, callback, 20);
}
...
int16_t *paranoia_read_limited(cdrom_paranoia *p, int *exit_ripping, void (*callback)(long, int),
		int max_retries) {
	...
	while (rv(root) == NULL || rb(root) > beginword
			|| (re(root) < endword + (MAX_SECTOR_OVERLAP * CD_FRAMEWORDS)
					&& p->enable
							& (PARANOIA_MODE_VERIFY | PARANOIA_MODE_OVERLAP))
			|| re(root) < endword) {

		// 2014.04.18 iriver-eddy
		if (*exit_ripping == 1)
			return NULL;
...

		{
			 ...
			 */
			c_block *new=i_read_c_block(p,exit_ripping,beginword,endword,callback);

			if (new) {
				if (p->enable
						& (PARANOIA_MODE_OVERLAP | PARANOIA_MODE_VERIFY)) {
