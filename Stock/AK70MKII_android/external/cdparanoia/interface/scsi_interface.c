...
// 2014.11.24 iriver-eddy
#define FAST_RIP_SPEED_SLEEP					(25 * 1000)
#define NORMAL_RIP_SPEED_SLEEP				(40 * 1000)
#define SLOW_RIP_SPEED_SLEEP					(60 * 1000)
#define EXTRA_SLOW_RIP_SPEED_SLEEP		(80 * 1000)
#define ULTRA_SLOW_RIP_SPEED_SLEEP		(100 * 1000)
...
static void tweak_SG_buffer(cdrom_drive *d) {
	...
	sprintf(buffer, "\tDMA scatter/gather table entries: %d\n\t"
			...
			table * (reserved / CD_FRAMESIZE_RAW));
	cdmessage(d, buffer);
	...
	// 2014.11.17 iriver-eddy
	ALOGD("### SG buffer size = %d ###", cur);
...
static int sg2_handle_scsi_cmd(cdrom_drive *d, unsigned char *cmd,
		unsigned int cmd_len, unsigned int in_size, unsigned int out_size,
	...
	if (bytecheck && out_size > in_size) {
		memset(d->private_data->sg_buffer + cmd_len + in_size, bytefill,
				out_size - in_size);
...
static int sgio_handle_scsi_cmd(cdrom_drive *d, unsigned char *cmd,
		unsigned int cmd_len, unsigned int in_size, unsigned int out_size,
...
//	hdr.timeout = 50000;
	...
	if (!in_size | out_size) {
		...
		if (bytecheck && d->interface != SGIO_SCSI_BUGGY1)
			hdr.dxfer_direction =
					out_size ? SG_DXFER_TO_FROM_DEV : SG_DXFER_NONE;
		else
...
static int handle_scsi_cmd(cdrom_drive *d, unsigned char *cmd,
		unsigned int cmd_len, unsigned int in_size, unsigned int out_size,
		unsigned char bytefill, int bytecheck, unsigned char *sense) {

	if (d->interface == SGIO_SCSI || d->interface == SGIO_SCSI_BUGGY1)
		return sgio_handle_scsi_cmd(d, cmd, cmd_len, in_size, out_size,
				bytefill, bytecheck, sense);
	return sg2_handle_scsi_cmd(d, cmd, cmd_len, in_size, out_size, bytefill,
			bytecheck, sense);
...
static int mode_sense_atapi(cdrom_drive *d, int size, int page) {
	...
	if (handle_scsi_cmd(d, cmd, 10, 0, size + 4, '\377', 1, sense))
		return (1);

	{
		unsigned char *b = d->private_data->sg_buffer;
		if (b[0])
			return (1); /* Handles only up to 256 bytes */
		if (b[6])
			return (1); /* Handles only up to 256 bytes */
...
static int mode_sense_scsi(cdrom_drive *d, int size, int page) {
	...
	if (handle_scsi_cmd(d, cmd, 6, 0, size, '\377', 1, sense))
		return (1);
...
static unsigned int get_orig_sectorsize(cdrom_drive *d) {
	...
	return (d->orgsize = ((int) (d->private_data->sg_buffer[10]) << 8)
			+ d->private_data->sg_buffer[11]);
}
...
static int scsi_read_toc2(cdrom_drive *d) {
	...
	d->disc_toc[i].dwStartSector = d->adjust_ssize
			* (be32_to_cpu(foo) + be32_to_cpu(bar));
...
static int scsi_set_speed(cdrom_drive *d, int speed) {
	...
	if (speed > 0)
		ALOGD("### %s() / speed is %dx ###", __FUNCTION__, speed);
	else
		ALOGD("### %s() / speed is full ###", __FUNCTION__);
...
static int i_read_28(cdrom_drive *d, void *p, long begin, long sectors, unsigned char *sense) {
	...
	cmd[5] = begin & 0xFF;
	cmd[8] = sectors;
	if ((ret = handle_scsi_cmd(d, cmd, 10, 0, sectors * CD_FRAMESIZE_RAW,
			'\177', 1, sense)))
		return (ret);
	if (p)
		memcpy(p, d->private_data->sg_buffer, sectors * CD_FRAMESIZE_RAW);
	return (0);
}
...
static int i_read_A8(cdrom_drive *d, void *p, long begin, long sectors, unsigned char *sense) {
	...
	cmd[5] = begin & 0xFF;
	cmd[9] = sectors;
	if ((ret = handle_scsi_cmd(d, cmd, 12, 0, sectors * CD_FRAMESIZE_RAW,
			'\177', 1, sense)))
		return (ret);
	if (p)
		memcpy(p, d->private_data->sg_buffer, sectors * CD_FRAMESIZE_RAW);
...
static int i_read_mmc(cdrom_drive *d, void *p, long begin, long sectors, unsigned char *sense) {
	...
	// /////////////////////////////////////////////////////////////////////////
	// 2014.11.24 iriver-eddy
	if (d->rip_speed > SCSI_READ_SPEED_INVALID) {
		// minimum sleep must be over 25ms
		if (d->rip_speed == SCSI_READ_SPEED_FAST) {
			ALOGV("\t### %s() / fast / sleep %d ms ###", __FUNCTION__, FAST_RIP_SPEED_SLEEP / 1000);
			usleep(FAST_RIP_SPEED_SLEEP);
		} else if (d->rip_speed == SCSI_READ_SPEED_NORMAL) {
			ALOGV("\t### %s() / normal / sleep %d ms ###", __FUNCTION__, NORMAL_RIP_SPEED_SLEEP / 1000);
			usleep(NORMAL_RIP_SPEED_SLEEP);
		} else if (d->rip_speed == SCSI_READ_SPEED_EXTRA_SLOW) {
			ALOGV("\t### %s() / extra slow / sleep %d ms ###", __FUNCTION__, EXTRA_SLOW_RIP_SPEED_SLEEP / 1000);
			usleep(EXTRA_SLOW_RIP_SPEED_SLEEP);
		} else if (d->rip_speed == SCSI_READ_SPEED_ULTRA_SLOW) {
			ALOGV("\t### %s() / ultra slow / sleep %d ms ###", __FUNCTION__, ULTRA_SLOW_RIP_SPEED_SLEEP / 1000);
			usleep(ULTRA_SLOW_RIP_SPEED_SLEEP);
		} else {
			ALOGV("\t### %s() / slow / sleep %d ms ###", __FUNCTION__, SLOW_RIP_SPEED_SLEEP / 1000);
			usleep(SLOW_RIP_SPEED_SLEEP);
		}
	}
	// /////////////////////////////////////////////////////////////////////////
...
static inline void LBA_to_MSF(long lba, unsigned char *M, unsigned char *S,
		unsigned char *F) {
	if (lba >= -150) {
		*M = (lba + 150) / (60 * 75);
		lba -= (*M) * 60 * 75;
...
static long scsi_read_map(cdrom_drive *d, void *p, long begin, long sectors,
	...
	while (1) {
		// 2014.11.18 iriver-eddy
//		ALOGD("### read_audio() ###");

		if ((err = map(d, (p ? buffer : NULL), begin, sectors, sense))) {
			// 2014.11.17 iriver-eddy
			ALOGW("### Failed read_audio() / exit code = %d ###", d->exit_ripping);
			...
			if (d->report_all) {
				...
				fprintf(stderr,
						"scsi_read error: sector=%ld length=%ld retry=%d\n",
						begin, sectors, retry_count);
				fprintf(stderr,
			...
			// /////////////////////////////////////////////////
			// 2014.11.14 iriver-eddy
			if (d->exit_ripping == 1) {
				ALOGE("### User cancelled ###");
				cderror(d, "11: user cancelled.\n");
				return (-11);
			}
			// /////////////////////////////////////////////////
			...
			if (buffer) {
				...
				if (i != sectors) {
					if (d->report_all) {
						char b[256];
						sprintf(b,
								"scsi_read underrun: pos=%ld len=%ld read=%ld retry=%d\n",
...
long scsi_read_28(cdrom_drive *d, void *p, long begin, long sectors) {
	return (scsi_read_map(d, p, begin, sectors, i_read_28));
}

long scsi_read_A8(cdrom_drive *d, void *p, long begin, long sectors) {
	return (scsi_read_map(d, p, begin, sectors, i_read_A8));
}

long scsi_read_D4_10(cdrom_drive *d, void *p, long begin, long sectors) {
	return (scsi_read_map(d, p, begin, sectors, i_read_D4_10));
}

long scsi_read_D4_12(cdrom_drive *d, void *p, long begin, long sectors) {
	return (scsi_read_map(d, p, begin, sectors, i_read_D4_12));
}

long scsi_read_D5(cdrom_drive *d, void *p, long begin, long sectors) {
	return (scsi_read_map(d, p, begin, sectors, i_read_D5));
}

long scsi_read_D8(cdrom_drive *d, void *p, long begin, long sectors) {
	return (scsi_read_map(d, p, begin, sectors, i_read_D8));
}

long scsi_read_mmc(cdrom_drive *d, void *p, long begin, long sectors) {
	return (scsi_read_map(d, p, begin, sectors, i_read_mmc));
}

long scsi_read_mmc2(cdrom_drive *d, void *p, long begin, long sectors) {
	return (scsi_read_map(d, p, begin, sectors, i_read_mmc2));
}

long scsi_read_mmc3(cdrom_drive *d, void *p, long begin, long sectors) {
	return (scsi_read_map(d, p, begin, sectors, i_read_mmc3));
}

long scsi_read_mmcB(cdrom_drive *d, void *p, long begin, long sectors) {
	return (scsi_read_map(d, p, begin, sectors, i_read_mmcB));
}

long scsi_read_mmc2B(cdrom_drive *d, void *p, long begin, long sectors) {
	return (scsi_read_map(d, p, begin, sectors, i_read_mmc2B));
}

long scsi_read_mmc3B(cdrom_drive *d, void *p, long begin, long sectors) {
	return (scsi_read_map(d, p, begin, sectors, i_read_mmc3B));
}

long scsi_read_msf(cdrom_drive *d, void *p, long begin, long sectors) {
	return (scsi_read_map(d, p, begin, sectors, i_read_msf));
}

long scsi_read_msf2(cdrom_drive *d, void *p, long begin, long sectors) {
	return (scsi_read_map(d, p, begin, sectors, i_read_msf2));
}

long scsi_read_msf3(cdrom_drive *d, void *p, long begin, long sectors) {
		...
		for (j = 0; j < 15; j++) {
			...
			for (i = 0; i < 5; i++) {
				...
				case 0:
					...
					es = "none    ";
					if (!densitypossible)
						i = 5; /* short circuit MMC style commands */
					break;
				case 1:
					d->density = 0;
...

				{
					...
					if (d->enable_cdda(d, 1) == 0) {
						for (k = 1; k <= d->tracks; k++) {
							if (cdda_track_audiop(d, k) == 1) {
								...
								if (d->read_audio(d, buff, sector, 1) > 0) {
									if ((lengthflag = count_2352_bytes(d))
											== 2352) {
										if (verify_nonzero(d)) {
											cdmessage(d,
													"\t\tCommand set FOUND!\n");
					...
					if (lengthflag > 0 && lengthflag < 2352) {
						char buffer[256];
						sprintf(buffer,
								"\t\tDrive returned at least one packet, but with\n"
										"\t\tincorrect size (%d)\n",
								lengthflag);
						cdmessage(d, buffer);
					}
					if (zeroflag) {
						char buffer[256];
						sprintf(buffer,
								"\t\tDrive returned %d packet(s), but contents\n"
										"\t\twere entirely zero\n", zeroflag);
						cdmessage(d, buffer);
					}
				}
...
static void check_cache(cdrom_drive *d) {
	...
	if (!(d->read_audio == scsi_read_mmc || d->read_audio == scsi_read_mmc2
			|| d->read_audio == scsi_read_mmc3
			|| d->read_audio == scsi_read_mmcB
			|| d->read_audio == scsi_read_mmc2B
			|| d->read_audio == scsi_read_mmc3B)) {
	...
	if (ioctl(fd, SG_EMULATED_HOST, &atapiret)) {
		...
		if (atapiret == 1) {
			if (d->interface == SGIO_SCSI) {
				...
				cdmessage(d,
						"\tDrive is ATAPI (using SG_IO host adaptor emulation with workarounds)\n");
			} else {
				cdmessage(d,
...
static void check_exceptions(cdrom_drive *d, exception *list) {
	...
	while (list[i].model) {
		if (!strncmp(list[i].model, d->drive_model, strlen(list[i].model))) {
			if (list[i].density)
				d->density = list[i].density;
			if (list[i].enable)
				d->enable_cdda = list[i].enable;
			if (list[i].read)
				d->read_audio = list[i].read;
			if (list[i].bigendianp != -1)
				d->bigendianp = list[i].bigendianp;
			return;
		}
		i++;
	}
...
// 2014.04.22 jhlim : added eject scsi command.
int scsi_eject(cdrom_drive *d) {
	unsigned char sense[SG_MAX_SENSE];

//	unsigned char cmd1[12] = { ALLOW_MEDIUM_REMOVAL, 0, 0, 0, 0, 0 };
	unsigned char cmd2[12] = { START_STOP, 0, 0, 0, 1, 0 };
	unsigned char cmd3[12] = { START_STOP, 0, 0, 0, 2, 0 };

//	handle_scsi_cmd(d, cmd1, 6, 0, 0, 0, 0, sense);
	handle_scsi_cmd(d, cmd2, 6, 0, 0, 0, 0, sense);
	return handle_scsi_cmd(d, cmd3, 6, 0, 0, 0, 0, sense);
}

// 2014.07.31 iriver-eddy
// Prevent eject during data reading
int scsi_eject_lock(cdrom_drive *d, int lock) {
	unsigned char sense[SG_MAX_SENSE];
	unsigned char cmd[6] = { ALLOW_MEDIUM_REMOVAL, 0, 0, 0, 0, 0 };

	cmd[1] = d->lun << 5;
	if (lock != 0)
		cmd[4] = 0x01;

	return handle_scsi_cmd(d, cmd, 6, 0, 0, 0, 0, sense);
}
...
int scsi_init_drive(cdrom_drive *d) {
	...
	d->fua = 0x0;
	if (d->is_atapi)
		d->lun = 0; /* it should already be; just to make sure */
	...
	if (!d->is_atapi)
		set_sectorsize(d, 2048); /* we really do want the
		 sector size at 2048 to begin.*/
	d->enable_cdda(d, 0);

	d->read_toc =
			(!memcmp(d->drive_model, "IMS", 3) && !d->is_atapi) ?
	...
	// 2014.11.24 iriver-eddy
	d->rip_speed = SCSI_READ_SPEED_INVALID;
	// 2017.07.07 iriver-eddy
	d->cdrom_type = CDROM_TYPE_V1;

	// 2014.11.17 iriver-eddy
/*
	if ((ret = verify_read_command(d))) {
		// 2014.11.17 iriver-eddy
		ALOGE("### scsi_init_drive() / verify_read_command() ###");
		return (ret);
	}
*/
	check_cache(d);
	...
	d->error_retry = 1;
	d->private_data->sg_hd = realloc(d->private_data->sg_hd,
			d->nsectors * CD_FRAMESIZE_RAW + SG_OFF + 128);
	d->private_data->sg_buffer = ((unsigned char *) d->private_data->sg_hd)
			+ SG_OFF;
	d->report_all = 1;
	// 2014.11.14 iriver-eddy
	d->exit_ripping = 0;
