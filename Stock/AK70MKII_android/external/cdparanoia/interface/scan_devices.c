...
static char *scsi_cdrom_prefixes[] = { "/dev/scd", "/dev/sr", NULL };
static char *scsi_generic_prefixes[] = { "/dev/sg", NULL };

static char *devfs_scsi_test = "/dev/scsi/";
static char *devfs_scsi_cd = "cd";
static char *devfs_scsi_generic = "generic";

static char *cdrom_devices[] = { "/dev/sg?", "/dev/cdrom", "/dev/cdroms/cdrom?", "/dev/hd?",
		"/dev/cdu31a", "/dev/cdu535", "/dev/sbpcd", "/dev/sbpcd?",
		"/dev/sonycd", "/dev/mcd", "/dev/sjcd",
		/* "/dev/aztcd", timeout is too long */
		"/dev/cm206cd", "/dev/gscd", "/dev/optcd", NULL };
...
cdrom_drive *cdda_find_a_cdrom(int messagedest, char **messages) {
	...
	while (cdrom_devices[i] != NULL) {
		printf("### cdrom_devices: %s ###", cdrom_devices[i]);
...
cdrom_drive *cdda_identify(const char *device, int messagedest,
		char **messages) {
	struct stat st;
	cdrom_drive *d = NULL;
...
cdrom_drive *cdda_identify_cooked(const char *dev, int messagedest,
	...
	case IDE3_MAJOR:
		...
		if (ioctl_ping_cdrom(fd)) {
			idmessage(messagedest, messages, "\t\tDevice %s is not a CDROM",
					device);
			close(fd);
			free(device);
			return (NULL);
	...
	case MATSUSHITA_CDROM4_MAJOR:
		...
		description =
				copystring(
						"non-ATAPI IDE-style Matsushita/Panasonic CR-5xx or compatible");
		break;
	case SANYO_CDROM_MAJOR:
		description = copystring(
				"Sanyo proprietary or compatible: NOT CDDA CAPABLE");
		break;
	case MITSUMI_CDROM_MAJOR:
	case MITSUMI_X_CDROM_MAJOR:
		description = copystring(
				"Mitsumi proprietary or compatible: NOT CDDA CAPABLE");
		break;
	case OPTICS_CDROM_MAJOR:
		description = copystring(
				"Optics Dolphin or compatible: NOT CDDA CAPABLE");
		break;
	case AZTECH_CDROM_MAJOR:
		description = copystring(
				"Aztech proprietary or compatible: NOT CDDA CAPABLE");
		break;
	...
	case CM206_CDROM_MAJOR:
		description = copystring(
				"Philips/LMS CM206 proprietary: NOT CDDA CAPABLE");
		break;
...
static char *scsi_match(const char *device, char **prefixes, char *devfs_test,
	...
	if (get_scsi_id(dev, &a)) {
		idperror(messagedest, messages,
				"\t\tDevice %s could not perform ioctl()", device);
...
static int verify_SG_version(cdrom_drive *d, int messagedest, char **messages) {
	...
	sprintf(buffer, "\tSG interface version %d.%d.%d; OK.", major, minor,
			version);
...
int check_sgio(const char *device, int messagedest, char **messages) {
	...
	if (fd < 0) {
		idperror(messagedest, messages,
				"\t\tCould not access device %s to test for SG_IO support",
				device);
		return 0;
	}
...
cdrom_drive *cdda_identify_scsi(const char *generic_device,
		const char *specialized_device, int messagedest, char **messages) {
	...
	if (generic_device) {
		...
		if ((int) (g_st.st_rdev >> 8) != SCSI_GENERIC_MAJOR) {
			idmessage(messagedest, messages,
					"\t\t%s is not a generic SCSI device", generic_device);
			return (NULL);
		}
	}
	...
	if (generic_device) {
		generic_device = test_resolve_symlink(generic_device, messagedest,
				messages);
	...
	if (use_sgio) {
		if (check_sgio(specialized_device, messagedest, messages)) {
			idmessage(messagedest, messages, "\t\tSG_IO device: %s",
					specialized_device);
		} else {
			idmessage(messagedest, messages,
	...
	if (!use_sgio) {
		...
		if (specialized_device) {
			...
			if (!generic_device || !specialized_device) {
				if (generic_device) {
					...
					generic_device =
							scsi_match(specialized_device,
									scsi_generic_prefixes, devfs_scsi_test,
									devfs_scsi_generic,
		...
		idmessage(messagedest, messages, "\t\tgeneric device: %s",
				generic_device);
		idmessage(messagedest, messages, "\t\tioctl device: %s",
				(specialized_device ? specialized_device : "not found"));
		if (specialized_device) {
			if (stat(specialized_device, &i_st)) {
				idperror(messagedest, messages,
	...
	if (p && !strncmp(p + 8, "TOSHIBA", 7) && !strncmp(p + 16, "CD-ROM", 6)
			&& p[0] == TYPE_DISK) {
		p[0] = TYPE_ROM;
		p[1] |= 0x80; /* removable */
	}
	...
	idmessage(messagedest, messages, "\nCDROM model sensed sensed: %s",
			d->drive_model);
	return (d);

	cdda_identify_scsi_fail: if (generic_device)
	...
	if (d) {
		if (d->private_data) {
			if (d->private_data->sg_hd)
				free(d->private_data->sg_hd);
			free(d->private_data);
		}
		free(d);
	}
