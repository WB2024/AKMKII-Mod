...
//2014.04.22
int cdda_eject(cdrom_drive *d)
{
  // 2014.04.22 iriver-eddy
  switch(d->interface){
  case SGIO_SCSI_BUGGY1:  
  case SGIO_SCSI:  
  case GENERIC_SCSI:  
    if(!scsi_eject(d)) return 0;
    break;
  }
  
  cderror(d,"406: eject error\n");
  return -406;
}

// 2014.07.31 iriver-eddy
int cdda_eject_lock(cdrom_drive *d, int lock)
{
  // 2014.04.22 iriver-eddy
  switch(d->interface){
  case SGIO_SCSI_BUGGY1:  
  case SGIO_SCSI:  
  case GENERIC_SCSI:  
    if(!scsi_eject_lock(d, lock)) return 0;
    break;
  }
  
  cderror(d,"406: eject lock error\n");
  return -406;
}
