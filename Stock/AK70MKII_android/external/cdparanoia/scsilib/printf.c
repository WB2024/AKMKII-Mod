...
#include <Windows.h>
#include <malloc.h>
	int		nCount = bp->ptr - bp->buf;

	if ( nCount > 0)
	{
		char* pTmp = malloc( nCount + 1 );

		bp->count += nCount;
		memcpy( pTmp, bp->buf, nCount );
		pTmp[ nCount ] = '\0';
...
extern int Xprintf(const char *form, ...)
{
    static char szBuffer[8192];
    va_list ap;
	memset(szBuffer,0x00,sizeof(szBuffer));
	va_start(ap, form);
    _vsnprintf(szBuffer, sizeof(szBuffer), form, ap);
...
extern int Xfprintf(FILE *file, const char *form, ...)
{
    static char szBuffer[8192];
    va_list ap;
	memset(szBuffer,0x00,sizeof(szBuffer));
